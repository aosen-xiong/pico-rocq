...
Definition extend_capability_after_write
  ...
  fun l => In Loc M l \/
    exists target D fdef,
      ...
      class_subtype CT (rctype (rt_type old)) D /\
...
Lemma safe_call_callee_rdm_root_origin :
  forall CT sGamma mt rGamma h x y m args sGamma'
...
Proof.
  intros CT sGamma mt rGamma h x y m args sGamma' vals ly cy runtime_mdef
  ...
  - assert (Hsignature : msignature runtime_mdef = msignature mdef).
    { eapply runtime_call_signature_agrees with (C := C).
      - exact Hwf.
      - exact Hget_y.
      - exact Href.
      - exact Hval_y.
...
Lemma safe_call_callee_active_component_colors :
  forall CT P Z M cutoff sGamma mt rGamma h x y m args sGamma'
    ...
    active_rdm_component_colors_separated CT h M Z
      ...
      (mkr_env (Iot ly :: vals)).
Proof.
  intros CT P Z M cutoff sGamma mt rGamma h x y m args sGamma' vals ly cy
...
Proof.
  ...
  have Hactive' := safe_call_callee_active_component_colors CT P Z M cutoff
    sGamma mt rGamma h x y m args sGamma' vals ly cy runtime_mdef Hwf
...
Proof.
  ...
  - exfalso. destruct Hscope; congruence.
  - assert (Hbase : class_subtype CT (rctype (rt_type old)) C).
    { eapply runtime_value_class_subtype with (y := x) (Ty := Tx) (ly := lx).
      - exact Hwf.
      - exact Hget_x.
      - exact Href.
      - exact Hvalx.
...
Proof.
  ...
  inversion Htyping; subst.
  all: exists Tx, C, fieldT; repeat split; try assumption.
  all: eapply runtime_value_class_subtype with (y := x) (Ty := Tx) (ly := lx).
  all: try exact Hwf; try exact Hget_x; try exact Href; try exact Hvalx.
  all: unfold r_basetype; rewrite Hobj; reflexivity.
Qed.
...
Proof.
  ...
  - eapply component_colors_after_typed_rdm_field_update_existing_set
      with (D := C) (runtime_fd := fieldT); eauto.
  - eapply component_colors_after_non_rdm_field_update
      with (C := C) (fieldT := fieldT).
    + exact Hobj.
    + exact Hbase.
...
Proof.
  intros CT P Z M cutoff sGamma mt rGamma h x f y lx old value sGamma'
    Tx C fieldT Hwf [Hforward [Hcomponents Hactive]] Htyping Hvalx Hobj
    Hgetx Href Hfield Hbase Hnotrdm.
  ...
  have Hextension := non_rdm_capability_extension_is_old CT h M lx old f
    value C fieldT Hbase Hfield Hnotrdm.
  intros capability_root zone_root Hcaproot
...
Lemma nonlocation_capability_extension_is_old :
  forall CT h M source old field value,
    (forall l, value <> Iot l) ->
    Included Loc
      (extend_capability_after_write CT h M source old field value) M.
Proof.
  intros CT h M source old field value Hnonlocation member
    [HinM | [target [D [runtime_fd [Hvalue Hrest]]]]].
  - exact HinM.
  - exfalso. exact (Hnonlocation target Hvalue).
Qed.

Lemma active_rdm_colors_after_nonlocation_field_write :
  forall CT P Z M cutoff sGamma rGamma h lx old f value sGamma',
    component_forward_history_state CT P Z M cutoff sGamma rGamma h ->
    sGamma' = sGamma ->
    runtime_getObj h lx = Some old ->
    (forall l, value <> Iot l) ->
    active_rdm_component_colors_separated CT
      ...
      sGamma' rGamma.
Proof.
  intros CT P Z M cutoff sGamma rGamma h lx old f value sGamma'
    [Hforward [Hcomponents Hactive]] -> Hobj Hnonlocation.
  intros capability_root zone_root Hcaproot
    [capability [Hcapability Hcapconnected]] Hzoneroot
    [protected [Hprotected Hzoneconnected]].
  apply (Hactive capability_root zone_root Hcaproot).
  - exists capability; split;
    [exact (nonlocation_capability_extension_is_old CT h M lx old f value
      Hnonlocation capability Hcapability)|];
    eapply mutable_connected_after_nonlocation_field_update_is_old; eauto.
  - exact Hzoneroot.
  - exists protected; split; [exact Hprotected|];
    eapply mutable_connected_after_nonlocation_field_update_is_old; eauto.
Qed.
...
Proof.
  intros CT P Z M cutoff sGamma mt rGamma h x f y lx old oldvalue value
    sGamma' Hwf Hstate Htyping Hscope Hvalx Hvaly Hobj Holdfield.
  destruct value as [|ly|n].
  - eapply active_rdm_colors_after_nonlocation_field_write.
    + exact Hstate.
  ...
  - destruct (typed_field_write_runtime_field_agreement CT sGamma mt rGamma
    ...
    + eapply active_rdm_colors_after_typed_non_rdm_field_write
        with (Tx := Tx) (C := C) (fieldT := fieldT); eauto.
    + eapply active_rdm_colors_after_typed_non_rdm_field_write
        with (Tx := Tx) (C := C) (fieldT := fieldT); eauto.
...
Proof.
  ...
  - split; [|exact Hconfheap].
    apply env_confined_update; [exact Hconfenv|].
    destruct value as [|result|n]; try trivial.
...
Proof.
  ...
  - intros y l Ty Htype Hval HinZ.
    destruct (appended_nonlocation_lookup_is_old sGamma rGamma T
      (default_value T) y Ty l (default_value_not_location T)
  ...
  - split; [|exact Hconfheap].
    ...
    + rewrite runtime_getVal_last in Hval.
      injection Hval as Hbad.
      exfalso. exact (default_value_not_location T l Hbad).
  ...
  - intros capability_root zone_root
    ...
    destruct (appended_nonlocation_lookup_is_old sGamma rGamma T
      (default_value T) yc Tc capability_root (default_value_not_location T)
      Hlength Htypec Hvalc) as [Holdtypec Holdvalc].
    destruct (appended_nonlocation_lookup_is_old sGamma rGamma T
      (default_value T) yz Tz zone_root (default_value_not_location T)
      Hlength Htypez Hvalz) as [Holdtypez Holdvalz].
