...
| Theorem 4, TS clause | Transitive-state preservation | Method call occurs in the paper's TS scope (`ConcreteImm` in Rocq); receiver and all parameters are safe | Each protected object still exists with the same runtime type, and every field retains its entry value | `ConcreteImmutability` in [ConcreteImmutability.v](ConcreteImmutability.v) |
