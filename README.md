...
| Theorem 1 safety support | Mutation exception cannot arise during a well-typed statement | Runtime config is well-formed; the statement is well-typed; assume its evaluation, including nested method bodies and sequences, produces `MUTATIONEXP` | Contradiction: no well-typed statement can produce the mutation exception | `well_typed_no_mutation_exp` in [WFNOMutationEXP.v](WFNOMutationEXP.v) |
