...
Lemma layered_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x e old value,
    layered_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SVarAss x e) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x e old value
    [Hlive [Hlayers Hcutoffs]] Htyping Hscope Hx Heval.
  have Hlive_post := live_history_after_assignment CT P Z cutoff authority
    sGamma mt rGamma h stack x e old value Hlive Htyping Hscope Hx Heval.
  split; [exact Hlive_post|]. split; [|exact Hcutoffs].
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 Hlive)).
  have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt
    rGamma h x e old value Hwf Htyping Hscope Hx Heval.
  eapply layered_colors_after_active_descent; eauto.
...
Lemma staged_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x e old value,
    staged_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SVarAss x e) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x e old value
    [Hlive [Hstaged Hcutoffs]] Htyping Hscope Hx Heval.
  have Hlive_post := live_history_after_assignment CT P Z cutoff authority
    sGamma mt rGamma h stack x e old value Hlive Htyping Hscope Hx Heval.
  split; [exact Hlive_post|]. split; [|exact Hcutoffs].
  have Hwf : wf_r_config CT sGamma rGamma h :=
  ...
  have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt
    rGamma h x e old value Hwf Htyping Hscope Hx Heval.
  eapply staged_colors_after_active_descent; eauto.
  intros location Hlocation.
  eapply assignment_live_reachability_is_old with
    (mt := mt) (x := x) (e := e) (old := old) (value := value);
    eauto.
...
Lemma authority_color_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x e old value,
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x e old value
    [Hlive [Hcolors Hcutoffs]] Htyping Hscope Hx Heval.
  have Hlive_post := live_history_after_assignment CT P Z cutoff authority
    sGamma mt rGamma h stack x e old value Hlive Htyping Hscope Hx Heval.
  split; [exact Hlive_post|]. split; [|exact Hcutoffs].
  have Hwf : wf_r_config CT sGamma rGamma h :=
  ...
  have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt
    rGamma h x e old value Hwf Htyping Hscope Hx Heval.
  eapply authority_colors_after_active_descent; eauto.
  intros location Hlocation.
  eapply assignment_live_reachability_is_old with
    (mt := mt) (x := x) (e := e) (old := old) (value := value);
    eauto.
...
Lemma principled_authority_color_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x e old value,
    principled_authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
    stmt_typing CT sGamma mt (SVarAss x e) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
    eval_expr CT rGamma h e value OK rGamma h ->
    principled_authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma
        (update_r_env_value rGamma x value)) stack tracked_depth h.
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x e old value
    [Hstate Hpending] Htyping Hscope Hx Heval.
  split.
  - eapply authority_color_history_after_assignment; eauto.
  - have Hwf : wf_r_config CT sGamma rGamma h :=
      proj1 (proj1 (proj2 (proj1 Hstate))).
    have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt
      rGamma h x e old value Hwf Htyping Hscope Hx Heval.
    eapply pending_call_stateful_authority_after_active_descent; eauto.
    intros location Hlocation.
    eapply assignment_live_reachability_is_old with
      (mt := mt) (x := x) (e := e) (old := old) (value := value);
      eauto.
...
Lemma principled_phased_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x e old value,
    principled_phased_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
    stmt_typing CT sGamma mt (SVarAss x e) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x e old value [Hlive [Hphased [Hcutoffs Hpending]]]
  ...
  - eapply phased_colors_after_active_descent; eauto.
    intros location Hlocation.
    eapply assignment_live_reachability_is_old with
      (mt := mt) (x := x) (e := e) (old := old) (value := value)
      (stack := []); eauto.
...
Lemma principled_potential_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x e old value,
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x e old value
    [Hstate Hpending] Htyping Hscope Hx Heval.
  split.
  - eapply potential_history_after_assignment; eauto.
  - have Hwf : wf_r_config CT sGamma rGamma h :=
      proj1 (proj1 (proj2 (proj1 Hstate))).
    have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt
      rGamma h x e old value Hwf Htyping Hscope Hx Heval.
    eapply pending_call_colors_after_active_descent; eauto.
    intros location Hlocation.
    eapply assignment_live_reachability_is_old with
      (mt := mt) (x := x) (e := e) (old := old) (value := value);
      eauto.
...
Lemma layered_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma',
    layered_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SLocal T x) sGamma' ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma'
    [Hlive [Hlayers Hcutoffs]] Htyping Hnone.
  have Hlive_post := live_history_after_local CT P Z cutoff authority
    sGamma mt rGamma h stack T x sGamma' Hlive Htyping Hnone.
  split; [exact Hlive_post|]. split; [|exact Hcutoffs].
  have Hwf : wf_r_config CT sGamma rGamma h :=
...
Lemma staged_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma',
    staged_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SLocal T x) sGamma' ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma'
    [Hlive [Hstaged Hcutoffs]] Htyping Hnone.
  have Hlive_post := live_history_after_local CT P Z cutoff authority
    sGamma mt rGamma h stack T x sGamma' Hlive Htyping Hnone.
  split; [exact Hlive_post|]. split; [|exact Hcutoffs].
  have Hwf : wf_r_config CT sGamma rGamma h :=
...
Lemma authority_color_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma',
    authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SLocal T x) sGamma' ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma'
    [Hlive [Hcolors Hcutoffs]] Htyping Hnone.
  have Hlive_post := live_history_after_local CT P Z cutoff authority
    sGamma mt rGamma h stack T x sGamma' Hlive Htyping Hnone.
  split; [exact Hlive_post|]. split; [|exact Hcutoffs].
  have Hwf : wf_r_config CT sGamma rGamma h :=
...
Lemma principled_authority_color_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    T x sGamma',
    principled_authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
    ...
    principled_authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma'
        (set_vars rGamma (vars rGamma ++ [Null_a]))) stack tracked_depth h.
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    T x sGamma'
    [Hstate Hpending] Htyping Hnone.
  split.
  - eapply authority_color_history_after_local; eauto.
  - have Hwf : wf_r_config CT sGamma rGamma h :=
      proj1 (proj1 (proj2 (proj1 Hstate))).
    have Hdescend := rdm_roots_descend_after_local CT sGamma mt rGamma h
      T x sGamma' Hwf Htyping Hnone.
    eapply pending_call_stateful_authority_after_active_descent; eauto.
    intros location Hlocation.
    eapply local_live_reachability_is_old with
      (mt := mt) (T := T) (x := x) (sGamma' := sGamma'); eauto.
Qed.

Lemma principled_phased_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    T x sGamma',
    principled_phased_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
    ...
    principled_phased_live_history_state CT P Z cutoff
      ...
      stack tracked_depth h.
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    T x sGamma' [Hlive [Hphased [Hcutoffs Hpending]]] Htyping Hnone.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 Hlive)).
  have Hdescend := rdm_roots_descend_after_local CT sGamma mt rGamma h
    T x sGamma' Hwf Htyping Hnone.
...
Lemma principled_local_mutable_rdm_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming
    x expression old value [Hstate Hcomponents] Htyping Hscope Hvalue Heval.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hstate))))).
  split.
  - eapply principled_phased_authority_history_after_assignment; eauto.
  - eapply active_mutable_rdm_components_after_assignment; eauto.
Qed.

Lemma principled_local_mutable_rdm_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    T x sGamma',
    principled_local_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming
    T x sGamma' [Hstate Hcomponents] Htyping Hnone.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hstate))))).
  split.
  - eapply principled_phased_authority_history_after_local; eauto.
  - eapply active_mutable_rdm_components_after_local; eauto.
Qed.

Lemma principled_live_mutable_rdm_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    x expression old value,
...
Proof.
  ...
  split.
  - eapply principled_phased_authority_history_after_assignment; eauto.
  - eapply live_mutable_rdm_components_after_active_descent; eauto.
    eapply rdm_roots_descend_after_assignment; eauto.
Qed.

Lemma principled_live_mutable_rdm_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    T x sGamma',
    principled_live_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming h ->
...
Lemma principled_separated_frozen_mutable_rdm_after_assignment :
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x expression old value Hstate Htyping Hscope Hvalue Heval.
  have Hfrozen := proj1 Hstate.
  ...
  { intros location Hlocation.
    ...
    eapply assignment_live_reachability_is_old with
      ...
      (stack := []); eauto.
    apply frame_owned_location_iff_active_live. exact Hlocation. }
  eapply principled_separated_frozen_mutable_rdm_after_active_descent;
    [exact Hstate| |exact Hdescend|exact Howned|].
  - eapply principled_phased_authority_history_after_assignment; eauto.
  - have Hlive := principled_live_mutable_rdm_history_after_assignment CT P Z
      cutoff authority sGamma mt rGamma h stack incoming x expression old
      value (conj Hmain (proj1 (proj2 Hstate))) Htyping Hscope Hvalue Heval.
    exact (proj2 Hlive).
Qed.

Lemma principled_separated_frozen_mutable_rdm_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    T x sGamma',
    principled_separated_frozen_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  ...
  eapply principled_separated_frozen_mutable_rdm_after_active_descent;
    [exact Hstate| |exact Hdescend|exact Howned|].
  - eapply principled_phased_authority_history_after_local; eauto.
  - have Hlive := principled_live_mutable_rdm_history_after_local CT P Z cutoff
      authority sGamma mt rGamma h stack incoming T x sGamma'
      (conj Hmain (proj1 (proj2 Hstate))) Htyping Hnone.
    exact (proj2 Hlive).
Qed.

Lemma principled_root_scoped_frozen_mutable_rdm_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x expression old value,
...
Proof.
  ...
  { intros location Hlocation.
    ...
    eapply assignment_live_reachability_is_old with
      ...
      (stack := []); eauto.
    apply frame_owned_location_iff_active_live. exact Hlocation. }
  eapply principled_root_scoped_frozen_mutable_rdm_after_active_descent;
    [exact Hstate| |exact Hdescend|exact Howned|].
  - eapply principled_phased_authority_history_after_assignment; eauto.
  - have Hlive := principled_live_mutable_rdm_history_after_assignment CT P Z
      cutoff authority sGamma mt rGamma h stack incoming x expression old
...
Lemma principled_root_scoped_frozen_mutable_rdm_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    T x sGamma',
    principled_root_scoped_frozen_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Lemma principled_complete_authority_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    tracked_depth x expression old value,
    principled_complete_authority_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming tracked_depth
      h ->
    stmt_typing CT sGamma mt (SVarAss x expression) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
    ...
    principled_complete_authority_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma
        (update_r_env_value rGamma x value))
      stack incoming tracked_depth h.
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming
    tracked_depth x expression old value [Hstate Hpending] Htyping Hscope
    Hvalue Heval.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hstate))))).
  have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt rGamma h
  ...
  split.
  - eapply principled_phased_authority_history_after_assignment; eauto.
  - eapply pending_call_phased_authority_after_active_descent; eauto.
    intros location Hlocation.
    eapply assignment_live_reachability_is_old with
      (mt := mt) (x := x) (e := expression) (old := old) (value := value);
      eauto.
...
Lemma principled_complete_authority_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    tracked_depth T x sGamma',
    principled_complete_authority_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming tracked_depth
      h ->
    stmt_typing CT sGamma mt (SLocal T x) sGamma' ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming
    tracked_depth T x sGamma' [Hstate Hpending] Htyping Hnone.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hstate))))).
  have Hdescend := rdm_roots_descend_after_local CT sGamma mt rGamma h
    T x sGamma' Hwf Htyping Hnone.
...
Lemma principled_potential_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma',
    principled_potential_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SLocal T x) sGamma' ->
...
Lemma principled_tracked_potential_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x expression old value,
    principled_tracked_potential_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
    stmt_typing CT sGamma mt (SVarAss x expression) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x expression old value [Hstate Hpending] Htyping Hscope Hvalue Heval.
  split.
  - eapply potential_history_after_assignment; eauto.
  - have Hwf : wf_r_config CT sGamma rGamma h :=
      proj1 (proj1 (proj2 (proj1 Hstate))).
    have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt
      rGamma h x expression old value Hwf Htyping Hscope Hvalue Heval.
    eapply tracked_pending_call_colors_after_active_descent; eauto.
    intros location Hlocation.
    eapply assignment_live_reachability_is_old with
      (mt := mt) (x := x) (e := expression) (old := old) (value := value);
      eauto.
...
Lemma principled_tracked_potential_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    T x sGamma',
    principled_tracked_potential_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    T x sGamma' [Hstate Hpending] Htyping Hnone.
  split.
  - eapply potential_history_after_local; eauto.
  - have Hwf : wf_r_config CT sGamma rGamma h :=
      proj1 (proj1 (proj2 (proj1 Hstate))).
...
Lemma layered_color_connected_after_field_update :
  forall CT h active stack lx old field value left right,
    runtime_getObj h lx = Some old ->
    layered_color_connected CT (update_field h lx field value)
      active stack left right ->
    layered_color_connected CT h active stack left right \/
    exists written,
      value = Iot written /\
      ((layered_color_connected CT h active stack left lx /\
        layered_color_connected CT h active stack written right) \/
       (layered_color_connected CT h active stack left written /\
        layered_color_connected CT h active stack lx right)).
Proof.
  intros CT h active stack lx old field value left right Hobj Hconnected.
  induction Hconnected.
  - destruct (layered_color_adjacent_after_field_update CT h active stack lx
      old field value x y Hobj H) as
      [Hold | [written [Hvalue [[-> ->] | [-> ->]]]]].
    + left. apply rt_step. exact Hold.
    + right. exists written. split; [exact Hvalue|]. left. split;
        apply rt_refl.
  ...
  - destruct IHHconnected1 as
    ...
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      right. exists written1. split; [exact Hvalue1|]. left. split;
        assumption.
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      left. eapply rt_trans; [exact Hxlx|exact Hlxyz].
    + right. exists written1. split; [exact Hvalue1|]. right. split.
      * exact Hxwritten1.
      * eapply rt_trans; eauto.
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      left. eapply rt_trans; [exact Hxwritten1|exact Hwritten2z].
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      right. exists written1. split; [exact Hvalue1|]. right. split;
        assumption.
Qed.
...
Lemma authority_color_adjacent_after_field_update :
  forall CT h active stack lx old field value left right,
    runtime_getObj h lx = Some old ->
    authority_color_adjacent CT (update_field h lx field value)
      active stack left right ->
    authority_color_adjacent CT h active stack left right \/
    exists written,
      value = Iot written /\
      ((left = lx /\ right = written) \/
       (left = written /\ right = lx)).
...
Lemma authority_color_connected_after_field_update :
  forall CT h active stack lx old field value left right,
    runtime_getObj h lx = Some old ->
    authority_color_connected CT (update_field h lx field value)
      active stack left right ->
    authority_color_connected CT h active stack left right \/
    exists written,
      value = Iot written /\
      ((authority_color_connected CT h active stack left lx /\
        authority_color_connected CT h active stack written right) \/
       (authority_color_connected CT h active stack left written /\
        authority_color_connected CT h active stack lx right)).
Proof.
  intros CT h active stack lx old field value left right Hobj Hconnected.
  induction Hconnected.
  - destruct (authority_color_adjacent_after_field_update CT h active stack
      lx old field value x y Hobj H) as
      [Hold | [written [Hvalue [[-> ->] | [-> ->]]]]].
    + left. apply rt_step. exact Hold.
    + right. exists written. split; [exact Hvalue|]. left. split;
        apply rt_refl.
...
Lemma authority_color_connected_after_null_field_update_is_old :
  forall CT h active stack lx old field left right,
    runtime_getObj h lx = Some old ->
    authority_color_connected CT (update_field h lx field Null_a)
      active stack left right ->
    authority_color_connected CT h active stack left right.
Proof.
  ...
  destruct (authority_color_connected_after_field_update CT h active stack
    lx old field Null_a left right Hobj Hconnected) as
    [Hold | [written [Hvalue Hnew]]].
  - exact Hold.
  - discriminate.
Qed.

Lemma authority_color_connected_after_non_rdm_field_update_is_old :
...
Proof.
  ...
  induction Hconnected.
  - destruct H as [Hheap | Hframe].
    + apply rt_step. left. destruct Hheap as [Hforward | Hbackward].
      * left. destruct (retained_edge_after_field_update CT h lx old field
          value x y Hobj Hforward) as
        ...
        -- assert (runtime_fd = fieldT).
           { eapply field_defs_agree_at_runtime_subtype with
               (C := rctype (rt_type old)) (D1 := D) (D2 := C); eauto. }
           subst runtime_fd. contradiction.
      * right. destruct (mutable_edge_after_field_update CT h lx old field
          value y x Hobj Hbackward) as
          [Hold | [Hsource [Hvalue [D [runtime_fd [Hruntime_base
...
Lemma layered_color_connected_after_null_field_update_is_old :
  forall CT h active stack lx old field left right,
    runtime_getObj h lx = Some old ->
    layered_color_connected CT (update_field h lx field Null_a)
      active stack left right ->
    layered_color_connected CT h active stack left right.
Proof.
  intros CT h active stack lx old field left right Hobj Hconnected.
...
Lemma layered_color_connected_after_non_rdm_field_update_is_old :
  forall CT h active stack lx old field value C fieldT left right,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
...
Proof.
  intros CT h active stack lx old field value C fieldT left right Hobj
    Hbase Hfield Hnot_rdm Hconnected.
  induction Hconnected.
  - destruct H as [Hmutable | [Hframe | Hreturn]].
    + eapply mutable_connected_is_layered_color_connected.
...
Lemma layered_color_connected_after_redundant_field_update_is_old :
  forall CT h active stack lx old field written left right,
    runtime_getObj h lx = Some old ->
    layered_color_connected CT h active stack lx written ->
    layered_color_connected CT
      (update_field h lx field (Iot written)) active stack left right ->
    layered_color_connected CT h active stack left right.
Proof.
  intros CT h active stack lx old field written left right Hobj Hredundant
    Hconnected.
  destruct (layered_color_connected_after_field_update CT h active stack lx
    old field (Iot written) left right Hobj Hconnected) as
    [Hold | [new_written [Hvalue
      [[Hleft_lx Hwritten_right] | [Hleft_written Hlx_right]]]]].
  - exact Hold.
  - injection Hvalue as <-.
    eapply rt_trans; [exact Hleft_lx|].
    eapply rt_trans; [exact Hredundant|exact Hwritten_right].
  - injection Hvalue as <-.
    eapply rt_trans; [exact Hleft_written|].
    eapply rt_trans.
    + eapply layered_color_connected_sym. exact Hredundant.
    + exact Hlx_right.
Qed.
...
Lemma retained_reachable_after_non_rdm_field_update_is_old :
  forall CT h lx old field value C fieldT root location,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
    mutability (ftype fieldT) <> RDM_f ->
    ...
    retained_mut_reachable CT
      (update_field h lx field value) root location ->
    retained_mut_reachable CT h root location.
Proof.
  intros CT h lx old field value C fieldT root location Hobj Hbase
    Hfield Hnot_rdm Hnot_mut Hreachable.
  induction Hreachable.
  - constructor.
  - eapply rmr_step.
    + exact IHHreachable.
    + destruct (retained_edge_after_field_update CT h lx old field value
        l2 l3 Hobj H) as [Hold | [Hsource [Hvalue
          [D [runtime_fd [Hruntime_base
            [Hruntime_field Hruntime_q]]]]]]].
      * exact Hold.
      * assert (runtime_fd = fieldT).
        { eapply field_defs_agree_at_runtime_subtype with
            (C := rctype (rt_type old)) (D1 := D) (D2 := C); eauto. }
...
Lemma staged_return_adjacent_after_field_update_is_old :
  forall h callee boundary lx field value left right,
    staged_return_adjacent (update_field h lx field value)
      callee boundary left right ->
    staged_return_adjacent h callee boundary left right.
Proof.
  intros h callee boundary lx field value left right
    [Hview [Hreturn [Hruntime Hroots]]].
  repeat split; try assumption.
  repeat rewrite r_muttype_update_field_preserve in Hruntime.
  exact Hruntime.
Qed.

Lemma phased_authority_colors_after_null_field_update :
  forall CT h active stack Z lx old field,
    runtime_getObj h lx = Some old ->
    phased_authority_colors_separated CT h Z active stack ->
    phased_authority_colors_separated CT (update_field h lx field Null_a)
      Z active stack.
Proof.
  intros CT h active stack Z lx old field Hobj Hseparated.
  eapply phased_authority_colors_after_graph_reflection;
    [| | | |exact Hseparated].
  ...
  - intros callee boundary left right Hedge.
    eapply staged_return_adjacent_after_field_update_is_old; eauto.
Qed.

Lemma phased_authority_colors_after_non_rdm_field_update :
  forall CT h active stack Z lx old field value C fieldT,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
    mutability (ftype fieldT) <> RDM_f ->
    mutability (ftype fieldT) <> Mut_f ->
    phased_authority_colors_separated CT h Z active stack ->
    phased_authority_colors_separated CT (update_field h lx field value)
      Z active stack.
Proof.
  intros CT h active stack Z lx old field value C fieldT Hobj Hbase Hfield
    Hnot_rdm Hnot_mut Hseparated.
  eapply phased_authority_colors_after_graph_reflection;
    [| | | |exact Hseparated].
  - intros frame location Howned.
    eapply frame_owned_after_non_rdm_field_update_is_old; eauto.
  - intros left right Hedge.
    eapply retained_edge_after_non_rdm_field_update_is_old; eauto.
  - intros left right Hedge.
    eapply mutable_edge_after_non_rdm_field_update_is_old; eauto.
  - intros callee boundary left right Hedge.
    eapply staged_return_adjacent_after_field_update_is_old; eauto.
Qed.

Lemma phased_colors_after_null_field_update :
  forall CT h active stack Z lx old field,
    runtime_getObj h lx = Some old ->
    phased_colors_separated CT h Z active stack ->
    phased_colors_separated CT (update_field h lx field Null_a)
      Z active stack.
Proof.
  intros CT h active stack Z lx old field Hobj Hseparated.
  eapply phased_colors_after_graph_reflection; [| | |exact Hseparated].
  - intros frame location Howned.
    unfold phase_frame_capability_set in *.
    eapply frame_owned_after_null_field_update_is_old; eauto.
  - intros frame left right Hconnected.
    eapply staged_frame_connected_after_null_field_update_is_old; eauto.
  - intros callee boundary left right Hconnected.
    eapply staged_return_connected_after_field_update_is_old; eauto.
Qed.

Lemma phased_colors_after_non_rdm_field_update :
  forall CT h active stack Z lx old field value C fieldT,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
    ...
    mutability (ftype fieldT) <> Mut_f ->
    phased_colors_separated CT h Z active stack ->
    phased_colors_separated CT (update_field h lx field value)
      Z active stack.
Proof.
  intros CT h active stack Z lx old field value C fieldT Hobj Hbase Hfield
    Hnot_rdm Hnot_mut Hseparated.
  ...
  - intros frame location Howned.
    ...
    eapply frame_owned_after_non_rdm_field_update_is_old; eauto.
  - intros frame left right Hconnected.
    eapply staged_frame_connected_after_non_rdm_field_update_is_old; eauto.
  - intros callee boundary left right Hconnected.
    eapply staged_return_connected_after_field_update_is_old; eauto.
Qed.

(** General one-phase transport for a materialized edge.  Post-update seeds
    need not themselves be old seeds; it is enough that each has an origin in
    the old phase closure.  If the new edge is traversed, the path restarts at
    its target, which is an explicitly old seed. *)
Lemma staged_frame_closure_after_field_update_with_seed_origins :
  forall CT h frame old_seeds new_seeds lx old field written,
    runtime_getObj h lx = Some old ->
    (forall seed,
      In Loc new_seeds seed ->
      In Loc (staged_frame_closure CT h frame old_seeds) seed) ->
    In Loc old_seeds lx ->
    In Loc old_seeds written ->
    Included Loc
      (staged_frame_closure CT (update_field h lx field (Iot written))
        frame new_seeds)
      (staged_frame_closure CT h frame old_seeds).
Proof.
  intros CT h frame old_seeds new_seeds lx old field written Hobj
    Hseed_origin Hlx Hwritten location [seed [Hseed Hpath]].
  ...
  - destruct (Hseed_origin seed Hseed) as
      [origin [Horigin Horigin_seed]].
...
Lemma phase_frame_capability_after_colored_field_update_has_old_origin :
  forall CT h frame old_incoming lx old field written location,
    runtime_getObj h lx = Some old ->
    In Loc
      (Union Loc old_incoming (phase_frame_capability_set CT h frame))
      written ->
    In Loc
    ...
    In Loc
      ...
      location.
Proof.
  intros CT h frame old_incoming lx old field written location Hobj
    Hwritten Howned.
  unfold phase_frame_capability_set in Howned.
  ...
  - exists root. split.
    + right. exists root. split; [exact Hroot|constructor].
    + eapply retained_reachable_is_staged_frame_connected. exact Hold.
  - injection Hvalue as <-. exists written. split.
    + exact Hwritten.
    + eapply retained_reachable_is_staged_frame_connected.
      exact Hwritten_location.
Qed.

Lemma phase_frame_seeds_after_colored_field_update_have_old_origins :
  forall CT h frame old_incoming new_incoming lx old field written,
    runtime_getObj h lx = Some old ->
    ...
    forall seed,
      In Loc
        (Union Loc new_incoming
          (phase_frame_capability_set CT
            (update_field h lx field (Iot written)) frame)) seed ->
      In Loc
        (staged_frame_closure CT h frame
          (Union Loc old_incoming
            (phase_frame_capability_set CT h frame))) seed.
Proof.
  intros CT h frame old_incoming new_incoming lx old field written Hobj
    Hincoming Hwritten seed Hseed.
  inversion Hseed; subst.
  - eapply staged_frame_closure_contains. left.
...
Lemma phased_live_color_set_after_active_colored_field_update_included :
  forall CT h active stack lx old field written,
    runtime_getObj h lx = Some old ->
    In Loc (phase_frame_capability_set CT h active) lx ->
    In Loc (phase_frame_capability_set CT h active) written ->
    In Loc
      (phase_frame_capability_set CT
        (update_field h lx field (Iot written)) active) lx ->
    In Loc
      (phase_frame_capability_set CT
        (update_field h lx field (Iot written)) active) written ->
    Included Loc
      (phased_live_color_set CT
        (update_field h lx field (Iot written)) active stack)
      (phased_live_color_set CT h active stack).
Proof.
  intros CT h active stack lx old field written Hobj Hold_lx Hold_written
    Hpost_lx Hpost_written.
  unfold phased_live_color_set.
  set (old_seeds := Union Loc (Empty_set Loc)
    (phase_frame_capability_set CT h active)).
  set (new_seeds := Union Loc (Empty_set Loc)
    (phase_frame_capability_set CT
      (update_field h lx field (Iot written)) active)).
  assert (Hseed_origins : forall seed,
      In Loc new_seeds seed ->
      In Loc (staged_frame_closure CT h active old_seeds) seed).
  { unfold new_seeds, old_seeds.
    eapply phase_frame_seeds_after_colored_field_update_have_old_origins.
    ...
    - right. exact Hold_written. }
  assert (Hold_seed_lx : In Loc old_seeds lx).
  { unfold old_seeds. right. exact Hold_lx. }
  assert (Hold_seed_written : In Loc old_seeds written).
  { unfold old_seeds. right. exact Hold_written. }
  assert (Hframe_included : Included Loc
      (staged_frame_closure CT
        (update_field h lx field (Iot written)) active new_seeds)
      (staged_frame_closure CT h active old_seeds)).
  { eapply staged_frame_closure_after_field_update_with_seed_origins; eauto. }
  ...
  - set (new_return := staged_return_closure
    ...
    { unfold new_return, old_return.
      eapply staged_return_closure_after_field_update_included.
      exact Hframe_included. }
    eapply phased_live_color_set_from_after_colored_field_update_included.
    + exact Hobj.
    + exact Hreturn_included.
    + unfold old_return.
      eapply staged_return_closure_contains.
    ...
    + unfold new_return.
      eapply staged_return_closure_contains.
      eapply staged_frame_closure_contains.
      unfold new_seeds. right. exact Hpost_lx.
    + unfold new_return.
      eapply staged_return_closure_contains.
      eapply staged_frame_closure_contains.
      unfold new_seeds. right. exact Hpost_written.
Qed.

Lemma phased_colors_after_active_colored_field_update :
  forall CT h active stack Z lx old field written,
    runtime_getObj h lx = Some old ->
    In Loc (phase_frame_capability_set CT h active) lx ->
    In Loc (phase_frame_capability_set CT h active) written ->
    In Loc
    ...
    In Loc
      (phase_frame_capability_set CT
        (update_field h lx field (Iot written)) active) written ->
    phased_colors_separated CT h Z active stack ->
    phased_colors_separated CT
      (update_field h lx field (Iot written)) Z active stack.
Proof.
  intros CT h active stack Z lx old field written Hobj Hold_lx Hold_written
    Hpost_lx Hpost_written Hseparated protected Hcolored Hprotected.
  apply (Hseparated protected); [|exact Hprotected].
  eapply phased_live_color_set_after_active_colored_field_update_included;
    eauto.
Qed.
...
Lemma phased_live_color_set_from_after_immutable_field_update_included :
  forall CT h active stack old_incoming new_incoming lx old field written,
    runtime_getObj h lx = Some old ->
    ...
    live_frames_authority_sound (update_field h lx field (Iot written))
      active stack ->
    Included Loc new_incoming old_incoming ->
    (forall seed,
      In Loc new_incoming seed ->
...
Proof.
  ...
  induction stack as [|boundary tail IH];
    intros active old_incoming new_incoming lx old field written Hobj
      Hframes_old Hsound_old Hframes_post Hsound_post Hincoming
      Hnew_runtime Hlx_immutable Hwritten_immutable; simpl.
  all: destruct Hframes_old as [Hactive_old Hstack_old].
  all: destruct Hsound_old as [Hactive_sound_old Hstack_sound_old].
  all: destruct Hframes_post as [Hactive_post Hstack_post].
  all: destruct Hsound_post as [Hactive_sound_post Hstack_sound_post].
  all: set (old_seeds := Union Loc old_incoming
      (phase_frame_capability_set CT h active)).
  all: set (new_seeds := Union Loc new_incoming
      (phase_frame_capability_set CT
        (update_field h lx field (Iot written)) active)).
  all: assert (Included Loc new_seeds old_seeds) as Hseeds by
    (intros location Hlocation; inversion Hlocation; subst;
     [left; apply Hincoming; exact H |
      right; eapply phase_frame_capability_after_immutable_field_update_is_old;
      eauto]).
  all: assert (forall seed,
      In Loc new_seeds seed ->
      r_muttype (update_field h lx field (Iot written)) seed = Some Mut_r)
    as Hseed_runtime by
    (intros seed Hseed; inversion Hseed; subst;
     [apply Hnew_runtime; exact H |
      eapply live_capability_members_runtime_mutable
        with (active := active) (stack := []);
      [split; [exact Hactive_post|constructor] |
       split; [exact Hactive_sound_post|constructor] |
       apply frame_owned_location_iff_active_live;
       unfold phase_frame_capability_set in H; exact H]]).
  all: assert (Included Loc
      (staged_frame_closure CT
        (update_field h lx field (Iot written)) active new_seeds)
      (staged_frame_closure CT h active old_seeds)) as Hframe_included by
  ...
  - set (new_return := staged_return_closure
    ...
    { unfold new_return, old_return.
      eapply staged_return_closure_after_field_update_included.
      exact Hframe_included. }
    assert (Hframe_runtime : forall location,
      In Loc
        (staged_frame_closure CT
          (update_field h lx field (Iot written)) active new_seeds) location ->
      r_muttype (update_field h lx field (Iot written)) location = Some Mut_r).
    { eapply staged_frame_closure_members_runtime_mutable; eauto. }
    assert (Hreturn_runtime : forall location,
      In Loc new_return location ->
      r_muttype (update_field h lx field (Iot written)) location = Some Mut_r).
    { unfold new_return.
      eapply staged_return_closure_members_runtime_mutable.
      exact Hframe_runtime. }
    eapply IH.
    + exact Hobj.
    + split.
    ...
    + split.
      * exact (Forall_inv Hstack_sound_old).
      * exact (Forall_inv_tail Hstack_sound_old).
    + split.
      * exact (Forall_inv Hstack_post).
      * exact (Forall_inv_tail Hstack_post).
    + split.
      * exact (Forall_inv Hstack_sound_post).
      * exact (Forall_inv_tail Hstack_sound_post).
    + exact Hreturn_included.
...
Lemma phased_colors_after_immutable_field_update :
  forall CT h active stack Z lx old field written,
    runtime_getObj h lx = Some old ->
    live_frames_wf CT h active stack ->
    live_frames_authority_sound h active stack ->
    ...
    r_muttype h written = Some Imm_r ->
    phased_colors_separated CT h Z active stack ->
    phased_colors_separated CT (update_field h lx field (Iot written))
      Z active stack.
Proof.
  intros CT h active stack Z lx old field written Hobj Hframes_old
    Hsound_old Hframes_post Hsound_post Hlx_immutable Hwritten_immutable
    Hseparated protected Hcolored Hprotected.
  apply (Hseparated protected); [|exact Hprotected].
  unfold phased_live_color_set in *.
  eapply phased_live_color_set_from_after_immutable_field_update_included;
    eauto.
  - intros location Hempty. inversion Hempty.
  - intros location Hempty. inversion Hempty.
Qed.

Lemma retained_reachable_after_null_field_update_is_old :
  forall CT h lx old field root location,
    runtime_getObj h lx = Some old ->
    retained_mut_reachable CT
      (update_field h lx field Null_a) root location ->
    retained_mut_reachable CT h root location.
Proof.
  intros CT h lx old field root location Hobj Hreachable.
  destruct (retained_reachable_after_field_update CT h lx old field Null_a
    root location Hobj Hreachable) as
    [Hold | [written [Hvalue Hnew]]].
  - exact Hold.
...
(** A field write whose source and non-null target are both owned by the
    executing side of every pending call cannot merge that side with a
    suspended caller color.  If a suspended caller acquired authority through
    the new edge, it already owned the source; since the source is also owned
    by the executing side, reflexivity contradicts the pre-state separation.
    The two path directions are then handled by the field-update normal form. *)
Lemma pending_call_colors_after_callee_colored_field_update :
  forall CT h active stack lx old field written,
    runtime_getObj h lx = Some old ->
    In Loc (live_capability_set CT h active []) lx ->
    In Loc (live_capability_set CT h active []) written ->
    pending_call_ownership_colors_separated CT h active stack ->
    pending_call_ownership_colors_separated CT
      (update_field h lx field (Iot written)) active stack.
Proof.
  intros CT h active stack lx old field written Hobj Hlx_active
    Hwritten_active Hpending boundary above below capability owned
    Hpartition Hchannel_free Howned_post Hcapability_post.
  have Hlx_owned :
      In Loc (live_capability_set CT h active above) lx.
  { eapply active_capability_lifts_to_stack. exact Hlx_active. }
  have Hwritten_owned :
      In Loc (live_capability_set CT h active above) written.
  { eapply active_capability_lifts_to_stack. exact Hwritten_active. }
  have Howned_old :
      In Loc (live_capability_set CT h active above) owned.
  { eapply live_capability_reachable_after_field_update_if_written_live;
      [exact Hobj| |exact Howned_post].
  ...
  have Hcapability_old :
      In Loc
        ...
        capability.
  { destruct Hcapability_post as
      [root [Hroot Hreachable]].
    destruct (retained_reachable_after_field_update CT h lx old field
      (Iot written) root capability Hobj Hreachable) as
      [Hold | [new_written
        [Hvalue [Hroot_lx Hwritten_capability]]]].
    - exists root. split; assumption.
    - injection Hvalue as <-.
      have Hlx_caller :
          In Loc
            (live_capability_set CT h boundary.(boundary_caller) below) lx.
      { exists root. split; assumption. }
      exfalso.
      apply (Hpending boundary above below lx lx Hpartition Hchannel_free
        Hlx_owned Hlx_caller lx).
      split; apply rt_refl. }
  intros common [Hcapability_common_post Howned_common_post].
  have Hcapability_origin :
      exists caller_capability,
        In Loc
          (live_capability_set CT h boundary.(boundary_caller) below)
          caller_capability /\
        potential_connected CT h active stack caller_capability common.
  { destruct (potential_connected_after_field_update CT h active stack lx old
      field (Iot written) capability common Hobj Hcapability_common_post) as
      [Hpath_old |
       [new_written [Hvalue
         [[Hcapability_lx Hwritten_common] |
          [Hcapability_written Hlx_common]]]]].
    - exists capability. split; assumption.
    - injection Hvalue as <-. exfalso.
      apply (Hpending boundary above below capability lx Hpartition
        Hchannel_free Hlx_owned Hcapability_old lx).
      split; [exact Hcapability_lx|apply rt_refl].
    - injection Hvalue as <-. exfalso.
      apply (Hpending boundary above below capability written Hpartition
        Hchannel_free Hwritten_owned Hcapability_old written).
      split; [exact Hcapability_written|apply rt_refl]. }
  have Howned_origin :
      exists callee_capability,
        In Loc (live_capability_set CT h active above) callee_capability /\
        potential_connected CT h active stack callee_capability common.
  { destruct (potential_connected_after_field_update CT h active stack lx old
      field (Iot written) owned common Hobj Howned_common_post) as
      [Hpath_old |
       [new_written [Hvalue
         [[Howned_lx Hwritten_common] |
          [Howned_written Hlx_common]]]]].
    - exists owned. split; assumption.
    - injection Hvalue as <-.
      exists written. split; assumption.
    - injection Hvalue as <-.
      exists lx. split; assumption. }
  destruct Hcapability_origin as
  ...
  apply (Hpending boundary above below caller_capability callee_capability
    Hpartition Hchannel_free Hcallee_capability Hcaller_capability common).
  split; assumption.
Qed.

Lemma tracked_pending_call_colors_after_callee_colored_field_update :
  forall CT h active stack tracked_depth lx old field written,
    runtime_getObj h lx = Some old ->
    In Loc (live_capability_set CT h active []) lx ->
    In Loc (live_capability_set CT h active []) written ->
    tracked_pending_call_ownership_colors_separated CT h active stack
      tracked_depth ->
    tracked_pending_call_ownership_colors_separated CT
      (update_field h lx field (Iot written)) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field written Hobj Hlx_active
    Hwritten_active Hpending boundary above below capability owned Hpartition
    Htracked Hchannel_free Howned_post Hcapability_post.
  have Hlx_owned : In Loc (live_capability_set CT h active above) lx.
  { eapply active_capability_lifts_to_stack. exact Hlx_active. }
  have Hwritten_owned :
      In Loc (live_capability_set CT h active above) written.
  { eapply active_capability_lifts_to_stack. exact Hwritten_active. }
  have Howned_old : In Loc (live_capability_set CT h active above) owned.
  { eapply live_capability_reachable_after_field_update_if_written_live;
      [exact Hobj| |exact Howned_post].
    intros candidate Hcandidate. injection Hcandidate as <-.
    exact Hwritten_owned. }
  have Hcapability_old :
      In Loc
        (live_capability_set CT h boundary.(boundary_caller) below)
        capability.
  { destruct Hcapability_post as [root [Hroot Hreachable]].
    destruct (retained_reachable_after_field_update CT h lx old field
      (Iot written) root capability Hobj Hreachable) as
      [Hold | [new_written [Hvalue [Hroot_lx Hwritten_capability]]]].
    - exists root. split; assumption.
    - injection Hvalue as <-.
      have Hlx_caller :
          In Loc
      ...
      { exists root. split; assumption. }
      exfalso.
      apply (Hpending boundary above below lx lx Hpartition Htracked
        Hchannel_free Hlx_owned Hlx_caller lx).
      split; apply rt_refl. }
  intros common [Hcapability_common_post Howned_common_post].
  have Hcapability_origin :
      exists caller_capability,
  ...
  { destruct (potential_connected_after_field_update CT h active stack lx old
    ...
    - exists capability. split; assumption.
    - injection Hvalue as <-. exfalso.
      apply (Hpending boundary above below capability lx Hpartition Htracked
        Hchannel_free Hlx_owned Hcapability_old lx).
      split; [exact Hcapability_lx|apply rt_refl].
    - injection Hvalue as <-. exfalso.
      apply (Hpending boundary above below capability written Hpartition
        Htracked Hchannel_free Hwritten_owned Hcapability_old written).
      split; [exact Hcapability_written|apply rt_refl]. }
  have Howned_origin :
      exists callee_capability,
        In Loc (live_capability_set CT h active above) callee_capability /\
  ...
  destruct Howned_origin as
    [callee_capability [Hcallee_capability Hcallee_common]].
  apply (Hpending boundary above below caller_capability callee_capability
    Hpartition Htracked Hchannel_free Hcallee_capability Hcaller_capability
    common).
  split; assumption.
Qed.
...
(** If every location newly owned by a live frame remains in the old
    boundary component of some location already owned by that same frame,
    then a redundant heap update cannot create a new boundary path.  Both
    directions in [Howned_origin] are intentional: frame ownership edges are
    symmetric even though the underlying potential graph is directed. *)
Lemma boundary_connected_after_field_update_if_edge_redundant :
  forall CT h active stack lx old field written left right,
    runtime_getObj h lx = Some old ->
    potential_connected CT h active stack lx written ->
    potential_connected CT h active stack written lx ->
    ...
    boundary_connected CT (update_field h lx field (Iot written))
      active stack left right ->
    boundary_connected CT h active stack left right.
Proof.
  intros CT h active stack lx old field written left right Hobj Hforward
    Hbackward Howned_origin Hconnected.
  induction Hconnected.
  - destruct H as [Hpotential | Hownership].
    + apply potential_connected_is_boundary_connected.
      ...
      apply rt_step. exact Hpotential.
    + destruct Hownership as [frame [Hlive [Hleft Hright]]].
      destruct (Howned_origin frame x Hlive Hleft) as
        [left_anchor [Hleft_anchor [Hleft_to_anchor Hanchor_to_left]]].
      destruct (Howned_origin frame y Hlive Hright) as
        [right_anchor [Hright_anchor [Hright_to_anchor Hanchor_to_right]]].
      eapply rt_trans; [exact Hleft_to_anchor|].
      eapply rt_trans.
      * eapply live_frame_owned_locations_boundary_connected
          with (frame := frame); eauto.
      * exact Hanchor_to_right.
  - apply rt_refl.
  - eapply rt_trans; eauto.
Qed.

Lemma boundary_connected_after_field_update_if_boundary_edge_redundant :
  forall CT h active stack lx old field written left right,
    runtime_getObj h lx = Some old ->
    boundary_connected CT h active stack lx written ->
    boundary_connected CT h active stack written lx ->
    (forall frame location,
      live_frame_member active stack frame ->
      frame_owned_location CT
        (update_field h lx field (Iot written)) frame location ->
...
Proof.
  ...
  induction Hconnected.
  - destruct H as [Hpotential | Hownership].
    + have Hpotential_path :
          potential_connected CT
            (update_field h lx field (Iot written)) active stack x y.
      { apply rt_step. exact Hpotential. }
      destruct (potential_connected_after_field_update CT h active stack
        lx old field (Iot written) x y Hobj Hpotential_path) as
        [Hold | [new_written [Hvalue [[Hleft_lx Hwritten_right] |
          [Hleft_written Hlx_right]]]]].
      * apply potential_connected_is_boundary_connected. exact Hold.
      * injection Hvalue as <-.
        eapply rt_trans.
        -- apply potential_connected_is_boundary_connected. exact Hleft_lx.
        -- eapply rt_trans; [exact Hforward|].
           apply potential_connected_is_boundary_connected.
      ...
      * injection Hvalue as <-.
        ...
        -- apply potential_connected_is_boundary_connected.
           exact Hleft_written.
        -- eapply rt_trans; [exact Hbackward|].
           apply potential_connected_is_boundary_connected. exact Hlx_right.
    + destruct Hownership as [frame [Hlive [Hleft Hright]]].
      destruct (Howned_origin frame x Hlive Hleft) as
        [left_anchor [Hleft_anchor [Hleft_to_anchor Hanchor_to_left]]].
      destruct (Howned_origin frame y Hlive Hright) as
...
Lemma frame_owned_after_active_owned_field_update_has_boundary_origin :
  forall CT h active stack frame lx old field written location,
    runtime_getObj h lx = Some old ->
    ...
    exists anchor,
      ...
      boundary_connected CT h active stack anchor location.
Proof.
  intros CT h active stack frame lx old field written location Hobj
    Hactive_lx Hactive_written Hframe_live
    [root [Hroot Hreachable]].
  destruct (retained_reachable_after_field_update CT h lx old field
    (Iot written) root location Hobj Hreachable) as
  ...
  - exists location. repeat split.
    ...
    + apply rt_refl.
  - injection Hvalue as <-.
    have Hframe_lx : frame_owned_location CT h frame lx.
    { exists root. split; assumption. }
    have Hactive_location : frame_owned_location CT h active location.
    { destruct Hactive_written as
        [written_root [Hwritten_root Hroot_written]].
      exists written_root. split; [exact Hwritten_root|].
      eapply retained_mut_reachable_transitive; eauto. }
    exists lx. split; [exact Hframe_lx|]. split.
    + eapply live_frame_owned_locations_boundary_connected
        with (frame := active).
      * constructor.
      * exact Hactive_location.
      * exact Hactive_lx.
    + eapply live_frame_owned_locations_boundary_connected
        with (frame := active).
      * constructor.
...
Lemma frame_owned_after_nonmutable_source_field_update_is_old :
  forall CT h active stack frame lx old field written location,
    wf_heap CT h ->
    live_frames_wf CT h active stack ->
    live_frames_authority_sound h active stack ->
    ...
    frame_owned_location CT h frame location.
Proof.
  intros CT h active stack frame lx old field written location Hheap
    Hframes Hsound Hobj Hlx_not_mut Hlive
    [root [Hroot Hreachable]].
  destruct (retained_reachable_after_field_update CT h lx old field
    (Iot written) root location Hobj Hreachable) as
    [Hold | [new_written
      [Hwritten_value [Hroot_lx Hwritten_location]]]].
  - exists root. split; assumption.
  - exfalso. apply Hlx_not_mut.
    have Hroot_live :
        In Loc (live_capability_set CT h active stack) root.
    { apply live_capability_iff_live_frame_owned.
    ...
    have Hroot_mut :=
      live_capability_members_runtime_mutable CT h active stack
        Hframes Hsound root Hroot_live.
...
Lemma boundary_connected_after_active_owned_field_update_is_old :
  forall CT h active stack lx old field written left right,
    runtime_getObj h lx = Some old ->
    frame_owned_location CT h active lx ->
    frame_owned_location CT h active written ->
    boundary_connected CT
      (update_field h lx field (Iot written))
      active stack left right ->
    boundary_connected CT h active stack left right.
Proof.
  intros CT h active stack lx old field written left right Hobj
    Hlx_owned Hwritten_owned Hconnected.
  have Hlx_written : boundary_connected CT h active stack lx written.
  { eapply live_frame_owned_locations_boundary_connected
      with (frame := active).
    - constructor.
    - exact Hlx_owned.
    - exact Hwritten_owned. }
  have Hwritten_lx : boundary_connected CT h active stack written lx.
  { eapply live_frame_owned_locations_boundary_connected
      with (frame := active).
    - constructor.
    - exact Hwritten_owned.
    - exact Hlx_owned. }
  eapply boundary_connected_after_field_update_if_boundary_edge_redundant;
    eauto.
  - intros frame location Hlive Howned.
...
Lemma boundary_connected_after_immutable_field_update_is_old_at_mutable_root :
  forall CT h active stack lx old field written left right,
    wf_heap CT h ->
    wf_heap CT (update_field h lx field (Iot written)) ->
    live_frames_wf CT h active stack ->
    ...
    boundary_connected CT h active stack left right.
Proof.
  intros CT h active stack lx old field written left right Hheap Hheap_post
    Hframes Hsound Hframes_post Hsound_post Hobj Hlx_imm Hwritten_imm
    Hlx_written Hwritten_lx Hleft_mut Hconnected.
  induction Hconnected.
  - destruct H as [Hpotential | Hownership].
    + apply potential_connected_is_boundary_connected.
      eapply potential_connected_after_field_update_if_edge_redundant;
        [exact Hobj|exact Hlx_written|exact Hwritten_lx|].
      apply rt_step. exact Hpotential.
    + apply rt_step. right.
      destruct Hownership as [frame [Hlive [Hleft_owned Hright_owned]]].
      exists frame. repeat split; try assumption.
  ...
  - apply rt_refl.
  - have Hmiddle_mut :
        r_muttype (update_field h lx field (Iot written)) y = Some Mut_r.
    { eapply boundary_connected_preserves_runtime_mutability;
        eauto. }
    eapply rt_trans.
    + apply IHHconnected1. exact Hleft_mut.
    + apply IHHconnected2. exact Hmiddle_mut.
Qed.
...
Lemma live_capability_after_redundant_field_update_has_old_authority_origin :
  forall CT h active stack lx old field written location,
    runtime_getObj h lx = Some old ->
    authority_color_connected CT h active stack lx written ->
    authority_color_connected CT h active stack written lx ->
    ...
    exists old_capability,
      In Loc (live_capability_set CT h active stack) old_capability /\
      authority_color_connected CT h active stack old_capability location.
Proof.
  intros CT h active stack lx old field written location Hobj Hforward
    Hbackward [root [Hroot Hreachable]].
  exists root. split.
  - exists root. split; [exact Hroot|constructor].
  - eapply authority_color_connected_after_redundant_field_update_is_old;
      [exact Hobj|exact Hforward|exact Hbackward|].
    eapply retained_reachable_is_authority_color_connected; eauto.
Qed.

Lemma authority_colors_after_redundant_field_update :
    ...
    (forall location,
      ...
      exists old_capability,
        ...
        authority_color_connected CT h active stack
          old_capability location) ->
    authority_colors_separated CT h M Z active stack ->
    authority_colors_separated CT
      (update_field h lx field (Iot written)) M' Z active stack.
Proof.
  intros CT h active stack lx old field written M' M Z Hobj Hforward
    Hbackward Horigin Hseparated capability protected Hcapability Hprotected
    Hconnected.
  ...
  eapply authority_color_connected_trans; [exact Hold_to_capability|].
  eapply authority_color_connected_after_redundant_field_update_is_old;
    eauto.
Qed.
...
Lemma tracked_pending_call_colors_after_null_field_update_directed :
  forall CT h active stack tracked_depth lx old field,
    runtime_getObj h lx = Some old ->
    tracked_pending_call_ownership_colors_separated CT h active stack
      tracked_depth ->
    tracked_pending_call_ownership_colors_separated CT
      (update_field h lx field Null_a) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field Hobj Hpending.
  eapply tracked_pending_call_colors_after_heap_graph_reflection;
    [| |exact Hpending].
  - intros frame substack location Hlocation.
    eapply live_capability_after_null_field_update_is_old; eauto.
  - intros frame substack left right Hconnected.
    eapply potential_connected_after_null_field_update_is_old; eauto.
Qed.

Lemma tracked_pending_call_colors_after_non_rdm_field_update_directed :
  forall CT h active stack tracked_depth lx old field value C fieldT,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
    mutability (ftype fieldT) <> RDM_f ->
    ...
    tracked_pending_call_ownership_colors_separated CT h active stack
      tracked_depth ->
    tracked_pending_call_ownership_colors_separated CT
      (update_field h lx field value) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field value C fieldT Hobj
    Hbase Hfield Hnot_rdm Hnot_mut Hpending.
  eapply tracked_pending_call_colors_after_heap_graph_reflection;
    [| |exact Hpending].
  - intros frame substack location Hlocation.
...
Lemma pending_call_stateful_authority_after_null_field_update :
  forall CT h active stack tracked_depth lx old field,
    runtime_getObj h lx = Some old ->
    pending_call_stateful_authority_separated CT h active stack
      tracked_depth ->
    pending_call_stateful_authority_separated CT
      (update_field h lx field Null_a) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field Hobj Hpending.
  eapply pending_call_stateful_authority_after_graph_reflection;
    [| | |exact Hpending].
  - intros frame substack location Hlocation.
    eapply live_capability_after_null_field_update_is_old; eauto.
  - intros left right Hedge.
...
Lemma pending_call_stateful_authority_after_non_rdm_field_update :
  forall CT h active stack tracked_depth lx old field value C fieldT,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
...
Lemma pending_call_phased_authority_after_null_field_update :
  forall CT h active stack tracked_depth lx old field,
    runtime_getObj h lx = Some old ->
    pending_call_phased_authority_separated CT h active stack
      tracked_depth ->
    pending_call_phased_authority_separated CT
      (update_field h lx field Null_a) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field Hobj Hpending.
  eapply pending_call_phased_authority_after_graph_reflection;
    [| | | |exact Hpending].
  - intros frame substack location Hlocation.
    eapply live_capability_after_null_field_update_is_old; eauto.
  - intros left right Hedge.
...
Lemma pending_call_phased_authority_after_non_rdm_field_update :
  forall CT h active stack tracked_depth lx old field value C fieldT,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
...
Lemma authority_flow_step_after_redundant_field_update_is_old :
  forall CT h active stack lx old field written source target,
    runtime_getObj h lx = Some old ->
    potential_frame_edge active stack lx written ->
    potential_frame_edge active stack written lx ->
    In Loc (live_capability_set CT h active stack) written ->
    authority_flow_step CT (update_field h lx field (Iot written))
      active stack source target ->
    authority_flow_connected CT h active stack source target.
Proof.
  intros CT h active stack lx old field written source target Hobj Hforward
    Hbackward Hwritten_live Hstep.
  inversion Hstep; subst.
  - destruct (retained_edge_after_field_update CT h lx old field
      (Iot written) left right Hobj H) as
      [Hold | [Hleft [Hvalue Hnew]]].
    + apply rt_step. apply authority_flow_retained. exact Hold.
    + injection Hvalue as <-. subst left.
      eapply rt_trans.
      * apply rt_step. apply authority_flow_powered_frame. exact Hforward.
      * apply rt_step. apply authority_flow_promote. exact Hwritten_live.
  - destruct (mutable_edge_after_field_update CT h lx old field
      (Iot written) right left Hobj H) as
      [Hold | [Hright [Hvalue Hnew]]].
    + apply rt_step. apply authority_flow_reverse_rdm. exact Hold.
    + injection Hvalue as <-. subst right.
      apply rt_step. apply authority_flow_powered_frame. exact Hbackward.
  - destruct (mutable_edge_after_field_update CT h lx old field
      (Iot written) left right Hobj H) as
      [Hold | [Hleft [Hvalue Hnew]]].
    + apply rt_step. apply authority_flow_neutral_rdm_forward. exact Hold.
    + injection Hvalue as <-. subst left.
      apply rt_step. apply authority_flow_neutral_frame. exact Hforward.
  - destruct (mutable_edge_after_field_update CT h lx old field
      (Iot written) right left Hobj H) as
      [Hold | [Hright [Hvalue Hnew]]].
    + apply rt_step. apply authority_flow_neutral_rdm_backward. exact Hold.
    + injection Hvalue as <-. subst right.
      apply rt_step. apply authority_flow_neutral_frame. exact Hbackward.
  - apply rt_step. apply authority_flow_powered_frame. exact H.
  - apply rt_step. apply authority_flow_neutral_frame. exact H.
...
Lemma authority_flow_connected_after_redundant_field_update_is_old :
  forall CT h active stack lx old field written source target,
    runtime_getObj h lx = Some old ->
    potential_frame_edge active stack lx written ->
    potential_frame_edge active stack written lx ->
    In Loc (live_capability_set CT h active stack) written ->
    authority_flow_connected CT (update_field h lx field (Iot written))
      active stack source target ->
    authority_flow_connected CT h active stack source target.
Proof.
  intros CT h active stack lx old field written source target Hobj Hforward
    Hbackward Hwritten_live Hconnected.
  induction Hconnected.
  - eapply authority_flow_step_after_redundant_field_update_is_old; eauto.
  - apply rt_refl.
  - eapply rt_trans; eauto.
Qed.

Lemma pending_call_colors_after_null_field_update_directed :
  forall CT h active stack lx old field,
    runtime_getObj h lx = Some old ->
    pending_call_ownership_colors_separated CT h active stack ->
    pending_call_ownership_colors_separated CT
...
Proof.
  intros CT h active stack lx old field Hobj Hpending.
  eapply pending_call_colors_after_heap_graph_reflection; [| |exact Hpending].
  - intros frame substack location Hlocation.
    eapply live_capability_after_null_field_update_is_old; eauto.
  - intros frame substack left right Hconnected.
    eapply potential_connected_after_null_field_update_is_old; eauto.
...
Lemma pending_call_colors_after_non_rdm_field_update_directed :
  forall CT h active stack lx old field value C fieldT,
    runtime_getObj h lx = Some old ->
    base_subtype CT (rctype (rt_type old)) C ->
    sf_def_rel CT C field fieldT ->
...
Lemma pending_call_colors_after_redundant_field_update_directed :
  forall CT h active stack lx old field written,
    runtime_getObj h lx = Some old ->
    potential_connected CT h active stack lx written ->
    potential_connected CT h active stack written lx ->
    ...
    pending_call_ownership_colors_separated CT
      (update_field h lx field (Iot written)) active stack.
Proof.
  intros CT h active stack lx old field written Hobj Hforward Hbackward
    Hpending boundary above below capability owned Hpartition Hchannel_free
    Howned_post Hcaller_post common [Hcaller_common_post Howned_common_post].
  destruct Hcaller_post as
    [caller_root [Hcaller_root Hcaller_reachable]].
  destruct Howned_post as
    [owned_root [Howned_root Howned_reachable]].
  have Hcaller_root_old :
      In Loc
        (live_capability_set CT h boundary.(boundary_caller) below)
        caller_root.
  { exists caller_root. split; [exact Hcaller_root|constructor]. }
  have Howned_root_old :
      In Loc (live_capability_set CT h active above) owned_root.
  { exists owned_root. split; [exact Howned_root|constructor]. }
  have Hcaller_root_to_capability :
      potential_connected CT h active stack caller_root capability.
  { eapply potential_connected_after_field_update_if_edge_redundant;
      [exact Hobj|exact Hforward|exact Hbackward|].
    eapply retained_reachable_is_potential_connected.
    exact Hcaller_reachable. }
  have Howned_root_to_owned :
      potential_connected CT h active stack owned_root owned.
  { eapply potential_connected_after_field_update_if_edge_redundant;
      [exact Hobj|exact Hforward|exact Hbackward|].
    eapply retained_reachable_is_potential_connected.
    exact Howned_reachable. }
  have Hcaller_common_old :
      potential_connected CT h active stack caller_root common.
  { eapply rt_trans; [exact Hcaller_root_to_capability|].
    eapply potential_connected_after_field_update_if_edge_redundant; eauto. }
  have Howned_common_old :
      potential_connected CT h active stack owned_root common.
  { eapply rt_trans; [exact Howned_root_to_owned|].
    eapply potential_connected_after_field_update_if_edge_redundant; eauto. }
  apply (Hpending boundary above below caller_root owned_root Hpartition
    Hchannel_free Howned_root_old Hcaller_root_old common).
  split; assumption.
Qed.

Lemma tracked_pending_call_colors_after_redundant_field_update_directed :
  forall CT h active stack tracked_depth lx old field written,
    runtime_getObj h lx = Some old ->
    potential_connected CT h active stack lx written ->
    potential_connected CT h active stack written lx ->
    ...
    tracked_pending_call_ownership_colors_separated CT
      (update_field h lx field (Iot written)) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field written Hobj Hforward
    Hbackward Hpending boundary above below capability owned Hpartition
    Htracked Hchannel_free Howned_post Hcaller_post common
    [Hcaller_common_post Howned_common_post].
  destruct Hcaller_post as
    [caller_root [Hcaller_root Hcaller_reachable]].
  destruct Howned_post as [owned_root [Howned_root Howned_reachable]].
  have Hcaller_root_old :
      In Loc
        (live_capability_set CT h boundary.(boundary_caller) below)
        caller_root.
  ...
  { eapply potential_connected_after_field_update_if_edge_redundant;
      [exact Hobj|exact Hforward|exact Hbackward|].
    eapply retained_reachable_is_potential_connected. exact Hcaller_reachable. }
  have Howned_root_to_owned :
      potential_connected CT h active stack owned_root owned.
  { eapply potential_connected_after_field_update_if_edge_redundant;
      [exact Hobj|exact Hforward|exact Hbackward|].
    eapply retained_reachable_is_potential_connected. exact Howned_reachable. }
  have Hcaller_common_old :
      potential_connected CT h active stack caller_root common.
  { eapply rt_trans; [exact Hcaller_root_to_capability|].
    eapply potential_connected_after_field_update_if_edge_redundant; eauto. }
  ...
  { eapply rt_trans; [exact Howned_root_to_owned|].
    eapply potential_connected_after_field_update_if_edge_redundant; eauto. }
  apply (Hpending boundary above below caller_root owned_root Hpartition
    Htracked Hchannel_free Howned_root_old Hcaller_root_old common).
  split; assumption.
Qed.
...
Lemma pending_call_colors_after_immutable_field_update_directed :
  forall CT h active stack lx old field written,
    live_frames_wf CT h active stack ->
    live_frames_authority_sound h active stack ->
    runtime_getObj h lx = Some old ->
    ...
    pending_call_ownership_colors_separated CT
      (update_field h lx field (Iot written)) active stack.
Proof.
  intros CT h active stack lx old field written Hframes Hsound Hobj Hlx_imm
    Hwritten_imm Hpending boundary above below capability owned Hpartition
    Hchannel_free Howned_post Hcaller_post common
    [Hcaller_common_post Howned_common_post].
  have Hheap : wf_heap CT h := proj1 (proj2 (proj1 Hframes)).
  have Howned_old :
      In Loc (live_capability_set CT h active above) owned.
  { destruct Howned_post as [root [Hroot Hreachable]].
    destruct (retained_reachable_after_field_update CT h lx old field
      (Iot written) root owned Hobj Hreachable) as
      [Hold | [new_written [Hvalue [Hroot_lx Hwritten_owned]]]].
    - exists root. split; assumption.
    - have Hroot_side :
          In Loc (live_capability_set CT h active above) root.
      { exists root. split; [exact Hroot|constructor]. }
      have Hroot_global :=
        live_call_partition_above_capability_is_live CT h active stack
          boundary above below root Hpartition Hroot_side.
      have Hroot_mut := live_capability_members_runtime_mutable CT h active
        stack Hframes Hsound root Hroot_global.
      have Hlx_mut := retained_reachable_preserves_runtime_context CT h root
        lx Mut_r Hheap Hroot_lx Hroot_mut.
      congruence. }
  have Hcaller_old :
      In Loc
        (live_capability_set CT h boundary.(boundary_caller) below)
        capability.
  { destruct Hcaller_post as [root [Hroot Hreachable]].
    destruct (retained_reachable_after_field_update CT h lx old field
      (Iot written) root capability Hobj Hreachable) as
      [Hold | [new_written [Hvalue [Hroot_lx Hwritten_capability]]]].
    - exists root. split; assumption.
    - have Hroot_side :
      ...
      { exists root. split; [exact Hroot|constructor]. }
      have Hroot_global :=
        live_call_partition_caller_capability_is_live CT h active stack
          boundary above below root Hpartition Hroot_side.
      have Hroot_mut := live_capability_members_runtime_mutable CT h active
        stack Hframes Hsound root Hroot_global.
      have Hlx_mut := retained_reachable_preserves_runtime_context CT h root
      ...
      congruence. }
  have Hcaller_global :=
    live_call_partition_caller_capability_is_live CT h active stack boundary
      above below capability Hpartition Hcaller_old.
  have Howned_global :=
    live_call_partition_above_capability_is_live CT h active stack boundary
      above below owned Hpartition Howned_old.
  have Hcaller_mut := live_capability_members_runtime_mutable CT h active
    stack Hframes Hsound capability Hcaller_global.
  have Howned_mut := live_capability_members_runtime_mutable CT h active
    stack Hframes Hsound owned Howned_global.
  have Hcaller_common_old :
      potential_connected CT h active stack capability common.
  { eapply potential_connected_after_immutable_field_update_is_old_at_mutable_root;
      eauto. }
  have Howned_common_old :
  ...
  { eapply potential_connected_after_immutable_field_update_is_old_at_mutable_root;
      eauto. }
  apply (Hpending boundary above below capability owned Hpartition
    Hchannel_free Howned_old Hcaller_old common).
  split; assumption.
Qed.

Lemma tracked_pending_call_colors_after_immutable_field_update_directed :
  forall CT h active stack tracked_depth lx old field written,
    live_frames_wf CT h active stack ->
    live_frames_authority_sound h active stack ->
    runtime_getObj h lx = Some old ->
    ...
    tracked_pending_call_ownership_colors_separated CT
      (update_field h lx field (Iot written)) active stack tracked_depth.
Proof.
  intros CT h active stack tracked_depth lx old field written Hframes Hsound
    Hobj Hlx_imm Hwritten_imm Hpending boundary above below capability owned
    Hpartition Htracked Hchannel_free Howned_post Hcaller_post common
    [Hcaller_common_post Howned_common_post].
  have Hheap : wf_heap CT h := proj1 (proj2 (proj1 Hframes)).
  have Howned_old : In Loc (live_capability_set CT h active above) owned.
  { destruct Howned_post as [root [Hroot Hreachable]].
    destruct (retained_reachable_after_field_update CT h lx old field
      ...
      [Hold | [new_written [Hvalue [Hroot_lx Hwritten_owned]]]].
    - exists root. split; assumption.
    - have Hroot_side : In Loc (live_capability_set CT h active above) root.
      { exists root. split; [exact Hroot|constructor]. }
      have Hroot_global :=
        live_call_partition_above_capability_is_live CT h active stack
          boundary above below root Hpartition Hroot_side.
...
Lemma layered_colors_after_graph_reflection :
  forall CT h h' active stack M M' Z,
    Included Loc M' M ->
    (forall left right,
      layered_color_connected CT h' active stack left right ->
    ...
    layered_colors_separated CT h' M' Z active stack.
Proof.
  intros CT h h' active stack M M' Z HM Hgraph Hseparated capability
    protected Hcapability Hprotected Hconnected.
  exact (Hseparated capability protected (HM capability Hcapability)
    Hprotected (Hgraph capability protected Hconnected)).
Qed.

Lemma authority_colors_after_graph_reflection :
  forall CT h h' active stack M M' Z,
    Included Loc M' M ->
    (forall left right,
      authority_color_connected CT h' active stack left right ->
...
Lemma authority_colors_after_m_colored_field_update :
  forall CT h active stack lx old field written M' M Z,
    runtime_getObj h lx = Some old ->
    Included Loc M' M ->
    In Loc M lx ->
...
Proof.
  intros CT h active stack lx old field written M' M Z Hobj HM Hlx
    ...
    Hconnected.
  destruct (authority_color_connected_after_field_update CT h active stack
    lx old field (Iot written) capability protected Hobj Hconnected) as
    [Hold | [new_written [Hvalue
      [[Hcap_lx Hwritten_protected] | [Hcap_written Hlx_protected]]]]].
  - exact (Hseparated capability protected (HM capability Hcapability)
      Hprotected Hold).
  - injection Hvalue as <-.
...
Lemma layered_colors_after_m_colored_field_update :
  forall CT h active stack lx old field written M' M Z,
    runtime_getObj h lx = Some old ->
    Included Loc M' M ->
    In Loc M lx ->
...
Proof.
  intros CT h active stack lx old field written M' M Z Hobj HM Hlx
    ...
    Hconnected.
  destruct (layered_color_connected_after_field_update CT h active stack lx
    old field (Iot written) capability protected Hobj Hconnected) as
    [Hold | [new_written [Hvalue
      [[Hcap_lx Hwritten_protected] | [Hcap_written Hlx_protected]]]]].
  - exact (Hseparated capability protected (HM capability Hcapability)
      Hprotected Hold).
...
Lemma layered_colors_after_immutable_field_update :
...
Proof.
  ...
  - injection Hvalue as <-.
    ...
    have Hheap_wf : wf_heap CT h := proj1 (proj2 (proj1 Hframes)).
    have Hlx_runtime :=
      layered_color_connected_preserves_runtime_mutability CT h active stack
        capability lx Mut_r Hframes Hheap_wf Hcap_lx Hcapability_runtime.
    congruence.
  - injection Hvalue as <-.
    have Hcapability_runtime := live_capability_members_runtime_mutable CT h
      active stack Hframes Hsound capability Hcapability_old.
...
Lemma authority_colors_after_immutable_field_update :
  forall CT h active stack lx old field written Z,
    live_frames_wf CT h active stack ->
    live_frames_authority_sound h active stack ->
    runtime_getObj h lx = Some old ->
...
Proof.
  ...
  - injection Hvalue as <-.
    ...
    have Hheap_wf : wf_heap CT h := proj1 (proj2 (proj1 Hframes)).
    have Hlx_runtime :=
      authority_color_connected_preserves_runtime_mutability CT h active stack
        capability lx Mut_r Hframes Hheap_wf Hcap_lx Hcapability_runtime.
    congruence.
  - injection Hvalue as <-.
    have Hcapability_runtime := live_capability_members_runtime_mutable CT h
...
Lemma frame_owned_after_immutable_source_field_update_is_old :
  forall CT h active stack frame lx old field written location,
    live_frames_wf CT h active stack ->
    live_frames_authority_sound h active stack ->
    live_frame_member active stack frame ->
    ...
    frame_owned_location CT h frame location.
Proof.
  intros CT h active stack frame lx old field written location Hframes Hsound
    Hframe_live Hobj Himmutable Howned.
  apply frame_owned_location_iff_active_live in Howned.
  apply frame_owned_location_iff_active_live.
  eapply live_capability_after_immutable_source_field_update_is_old
    with (active := frame) (stack := []); eauto.
  - split.
    + eapply live_frame_member_wf; eauto.
    + constructor.
  - split.
    + eapply live_frame_member_authority_sound; eauto.
    + constructor.
Qed.
...
Lemma authority_color_history_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x field y
    sGamma' rGamma' h',
    authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x field y
    sGamma' rGamma' h' [Hlive [Hcolors Hcutoffs]]
    Htyping Hscope Heval.
  assert (HsGamma : sGamma' = sGamma) by
    (inversion Htyping; reflexivity).
  assert (HrGamma : rGamma' = rGamma) by
    (inversion Heval; reflexivity).
  subst sGamma' rGamma'.
  assert (Hcolors_post : authority_colors_separated CT h'
      (live_capability_set CT h'
        (mk_watched_frame authority sGamma rGamma) stack) Z
      (mk_watched_frame authority sGamma rGamma) stack).
  {
    ...
    inversion Heval; subst.
    destruct val_y as [|written].
    - eapply authority_colors_after_graph_reflection
        with (M := live_capability_set CT h
          (mk_watched_frame authority sGamma rGamma) stack).
      + intros location Hlocation.
        eapply live_capability_after_null_field_update_is_old; eauto.
      + intros left right Hconnected.
        eapply authority_color_connected_after_null_field_update_is_old;
          eauto.
      + exact Hcolors.
    - destruct (typed_field_write_runtime_field_agreement CT sGamma mt rGamma
        h x field y loc_x o sGamma Hwf Htyping Hval_x Hobj) as
      ...
      + destruct (typed_runtime_mut_field_write_subtyping CT sGamma mt rGamma
        ...
        { eapply typed_mut_root_is_live_capability.
          exists y, Ty. repeat split; assumption. }
        eapply authority_colors_after_m_colored_field_update
          with (M := live_capability_set CT h
            (mk_watched_frame authority sGamma rGamma) stack).
        * exact Hobj.
        * intros location Hlocation.
      ...
      + eapply authority_colors_after_graph_reflection
        ...
        * intros location Hlocation.
          eapply live_capability_after_non_rdm_field_update_is_old
            with (C := sctype Tx) (fieldT := fieldT); eauto; discriminate.
        * intros left right Hconnected.
          eapply authority_color_connected_after_non_rdm_field_update_is_old
      ...
      + destruct (typed_runtime_rdm_field_write_subtyping CT sGamma mt rGamma
        ...
        * have Hloc_x_immutable : r_muttype h loc_x = Some Imm_r.
          ...
          { eapply typed_imm_root_runtime_immutable; [exact Hwf|].
            exists y, Ty. repeat split; assumption. }
          eapply authority_colors_after_immutable_field_update; eauto.
        * have Hforward : authority_color_connected CT h
              (mk_watched_frame authority sGamma rGamma) stack loc_x written.
          { apply rt_step. right.
            ...
            - exists x, Tx'. repeat split; assumption.
            - exists y, Ty. repeat split; assumption. }
          have Hbackward : authority_color_connected CT h
              (mk_watched_frame authority sGamma rGamma) stack written loc_x.
          { apply rt_step. right.
            exists (mk_watched_frame authority sGamma rGamma).
            split; [constructor|]. split.
            - exists y, Ty. repeat split; assumption.
            - exists x, Tx'. repeat split; assumption. }
          eapply authority_colors_after_redundant_field_update
            with (M := live_capability_set CT h
              (mk_watched_frame authority sGamma rGamma) stack).
          -- exact Hobj.
          -- exact Hforward.
          ...
          -- intros location Hlocation.
             eapply
               live_capability_after_redundant_field_update_has_old_authority_origin;
               eauto.
          -- exact Hcolors.
      + eapply authority_colors_after_graph_reflection
          with (M := live_capability_set CT h
  ...
  - eapply live_history_after_field_write_given_colors; eauto.
    ...
    + intros location Hlocation Hprotected.
      apply (Hcolors_post location location Hlocation Hprotected).
      apply rt_refl.
  - split; [exact Hcolors_post|].
    eapply live_boundary_cutoffs_valid_heap_growth; [|exact Hcutoffs].
    eapply eval_stmt_preserves_heap_domain_simple; exact Heval.
Qed.

Lemma principled_potential_history_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x field y
    sGamma' rGamma' h',
    principled_potential_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x field y
    sGamma' rGamma' h' [Hstate Hpending] Htyping Hscope Heval.
  have Hpost := potential_history_after_field_write CT P Z cutoff authority
    sGamma mt rGamma h stack x field y sGamma' rGamma' h'
    Hstate Htyping Hscope Heval.
  split; [exact Hpost|].
  assert (HsGamma : sGamma' = sGamma) by
    (inversion Htyping; reflexivity).
  assert (HrGamma : rGamma' = rGamma) by
  ...
  inversion Heval; subst.
  destruct val_y as [|written].
  - eapply pending_call_colors_after_null_field_update_directed; eauto.
  - destruct (typed_field_write_runtime_field_agreement CT sGamma mt rGamma
      h x field y loc_x o sGamma Hwf Htyping Hval_x Hobj) as
      [Tx [fieldT [Hgetx [Hfield_definition Hruntime_base]]]].
    destruct (mutability (ftype fieldT)) eqn:Hfield_mutability.
    + destruct (typed_runtime_mut_field_write_subtyping CT sGamma mt rGamma
      ...
      destruct (safe_mut_write_endpoint_qualifiers CT sGamma rGamma h x y
        loc_x written Tx' Ty (f_base_type (ftype fieldT)) Hwf Hgetx' Hgety
        Hval_x Hval_y Hsub) as [Hreceiver_mut Hvalue_mut].
      eapply pending_call_colors_after_callee_colored_field_update;
        [exact Hobj| | |exact Hpending].
      * eapply typed_mut_root_is_live_capability.
        exists x, Tx'. repeat split; assumption.
      * eapply typed_mut_root_is_live_capability.
        exists y, Ty. repeat split; assumption.
    + eapply pending_call_colors_after_non_rdm_field_update_directed
        with (C := sctype Tx) (fieldT := fieldT); eauto; discriminate.
    + destruct (typed_runtime_rdm_field_write_subtyping CT sGamma mt rGamma
        h x field y loc_x o (sctype Tx) fieldT sGamma Hwf Htyping Hscope
        Hval_x Hobj Hruntime_base Hfield_definition Hfield_mutability) as
      ...
      * have Hloc_x_imm : r_muttype h loc_x = Some Imm_r.
        ...
        { eapply typed_imm_root_runtime_immutable; [exact Hwf|].
          exists y, Ty. repeat split; assumption. }
        eapply pending_call_colors_after_immutable_field_update_directed;
          eauto.
      * have Hforward : potential_connected CT h
            (mk_watched_frame authority sGamma rGamma) stack loc_x written.
        { eapply live_frame_rdm_roots_potentially_connected
            with (frame := mk_watched_frame authority sGamma rGamma).
          - constructor.
          - exists x, Tx'. repeat split; assumption.
          - exists y, Ty. repeat split; assumption. }
        have Hbackward : potential_connected CT h
            (mk_watched_frame authority sGamma rGamma) stack written loc_x.
        { eapply live_frame_rdm_roots_potentially_connected
            with (frame := mk_watched_frame authority sGamma rGamma).
          - constructor.
          - exists y, Ty. repeat split; assumption.
          - exists x, Tx'. repeat split; assumption. }
        eapply pending_call_colors_after_redundant_field_update_directed;
          eauto.
    + eapply pending_call_colors_after_non_rdm_field_update_directed
        with (C := sctype Tx) (fieldT := fieldT); eauto; discriminate.
Qed.

Lemma principled_tracked_potential_history_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack tracked_depth
    x field y sGamma' rGamma' h' [Hstate Hpending] Htyping Hscope Heval.
  split.
  - eapply potential_history_after_field_write; eauto.
  - assert (HsGamma : sGamma' = sGamma) by
      (inversion Htyping; reflexivity).
    assert (HrGamma : rGamma' = rGamma) by
      (inversion Heval; reflexivity).
    ...
    inversion Heval; subst.
    destruct val_y as [|written].
    + eapply tracked_pending_call_colors_after_null_field_update_directed;
        eauto.
    + destruct (typed_field_write_runtime_field_agreement CT sGamma mt
      ...
      * destruct (typed_runtime_mut_field_write_subtyping CT sGamma mt
        ...
        eapply tracked_pending_call_colors_after_callee_colored_field_update;
          [exact Hobj| | |exact Hpending].
        -- eapply typed_mut_root_is_live_capability.
           exists x, Tx'. repeat split; assumption.
        -- eapply typed_mut_root_is_live_capability.
           exists y, Ty. repeat split; assumption.
      * eapply tracked_pending_call_colors_after_non_rdm_field_update_directed
          with (C := sctype Tx) (fieldT := fieldT); eauto; discriminate.
      * destruct (typed_runtime_rdm_field_write_subtyping CT sGamma mt
        ...
        -- have Hloc_x_imm : r_muttype h loc_x = Some Imm_r.
           ...
           { eapply typed_imm_root_runtime_immutable; [exact Hwf|].
             exists y, Ty. repeat split; assumption. }
           eapply tracked_pending_call_colors_after_immutable_field_update_directed;
             eauto.
        -- have Hforward : potential_connected CT h
              (mk_watched_frame authority sGamma rGamma) stack loc_x written.
           { eapply live_frame_rdm_roots_potentially_connected
           ...
           { eapply live_frame_rdm_roots_potentially_connected
             ...
             - exists y, Ty. repeat split; assumption.
             - exists x, Tx'. repeat split; assumption. }
           eapply tracked_pending_call_colors_after_redundant_field_update_directed;
             eauto.
      * eapply tracked_pending_call_colors_after_non_rdm_field_update_directed
          with (C := sctype Tx) (fieldT := fieldT); eauto; discriminate.
Qed.
...
(** Old-location color origin for a well-typed field update.  There is no
    cutoff condition because a field update allocates nothing. *)
Lemma executing_authority_colors_after_typed_field_write_covered :
  forall CT authority sGamma mt rGamma h incoming x field y
    sGamma' rGamma' h',
    wf_r_config CT sGamma rGamma h ->
    ...
    forall mode location,
      ...
      exists old_mode,
        ...
        In authority_flow_state
          ...
          (old_mode, location).
Proof.
  intros CT authority sGamma mt rGamma h incoming x field y sGamma'
    rGamma' h' Hwf Hold_runtime Htyping Hscope Heval.
  assert (HsGamma : sGamma' = sGamma) by
    (inversion Htyping; reflexivity).
  assert (HrGamma : rGamma' = rGamma) by
    (inversion Heval; reflexivity).
  ...
  inversion Heval; subst.
  destruct val_y as [|written].
  - eapply executing_authority_colors_after_graph_reflection_covered.
    + intros location Howned.
      eapply frame_owned_after_null_field_update_is_old; eauto.
    + intros left right Hedge.
      eapply retained_edge_after_null_field_update_is_old; eauto.
  ...
  - destruct (typed_field_write_runtime_field_agreement CT sGamma mt
    ...
    + destruct (typed_runtime_mut_field_write_subtyping CT sGamma mt
      ...
      destruct (safe_mut_write_endpoint_qualifiers CT sGamma rGamma h x y
        loc_x written Tx' Ty (f_base_type (ftype fieldT)) Hwf Hgetx'
        Hgety Hval_x Hval_y Hsub) as [Hreceiver_mut Hvalue_mut].
      eapply executing_authority_colors_after_safe_field_update_covered;
        [exact Hobj|exact Hold_runtime|].
      apply authority_safe_field_mutable.
      * apply frame_owned_location_iff_active_live.
        eapply typed_mut_root_is_live_capability.
        exists x, Tx'. repeat split; assumption.
...
Lemma principled_frozen_authority_history_after_field_write_classifies :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    snapshots x field y sGamma' rGamma' h',
    principled_frozen_authority_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  ...
  destruct (typed_field_write_component_effect CT authority sGamma mt
    rGamma h x field y sGamma' rGamma' h' Hwf Htyping Hscope Heval) as
    [[Hruntimes [Hmutable [Hretained Howned]]] | Hsafe].
  - left. eapply principled_frozen_authority_history_after_field_write_reflection;
      eauto.
  - right. exact Hsafe.
Qed.
...
Lemma principled_local_mutable_rdm_history_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming
    x field y sGamma' rGamma' h',
    principled_local_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming h ->
...
Proof.
  ...
  - have Hwf : wf_r_config CT sGamma rGamma h :=
    ...
    inversion Heval; subst.
    destruct val_y as [|written].
    + eapply active_mutable_rdm_components_after_graph_reflection.
      * intros location. apply r_muttype_update_field_preserve.
      * intros source target Hedge.
        eapply mutable_edge_after_null_field_update_is_old; eauto.
      * exact Hcomponents.
    + destruct (typed_field_write_runtime_field_agreement CT sGamma mt
      ...
      * destruct (typed_runtime_mut_field_write_subtyping CT sGamma mt
        ...
        destruct (safe_mut_write_endpoint_qualifiers CT sGamma rGamma h x y
          loc_x written Tx' Ty (f_base_type (ftype fieldT)) Hwf Hgetx'
          Hgety Hval_x Hval_y Hsub) as [Hreceiver_mut Hvalue_mut].
        eapply active_mutable_rdm_components_after_safe_field_update;
          [exact Hstate|exact Hcomponents|exact Hobj|].
        apply authority_safe_field_mutable.
        -- apply frame_owned_location_iff_active_live.
           eapply typed_mut_root_is_live_capability.
           exists x, Tx'. repeat split; assumption.
...
Lemma principled_separated_frozen_mutable_rdm_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h',
    principled_separated_frozen_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h' Hstate Htyping Hscope Heval.
  destruct Hstate as [Hfrozen [Hcomponents Hseparated]].
  have Hmain := proj1 Hfrozen.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hmain))))).
  have Heffect := typed_field_write_component_effect CT authority sGamma mt
    rGamma h x field y sGamma' rGamma' h' Hwf Htyping Hscope Heval.
  have Hfull_post := principled_frozen_authority_history_after_field_write CT
    P Z cutoff authority sGamma mt rGamma h stack incoming snapshots x field
    y sGamma' rGamma' h' Hfrozen Htyping Hscope Heval.
  have Hlive_post := principled_live_mutable_rdm_history_after_field_write CT
    P Z cutoff authority sGamma mt rGamma h stack incoming x field y sGamma'
    rGamma' h' (conj Hmain Hcomponents) Htyping Hscope Heval.
  ...
  destruct Heffect as
    [[Hruntimes [Hmutable [Hretained Howned]]] |
     [lx [old [written [Hheap [Hobj Hendpoints]]]]]].
  - eapply frozen_caller_snapshots_independently_separated_after_graph_reflection;
      eauto.
    exact (proj1 (proj2 (proj2 (proj2 Hfrozen)))).
  - subst h'.
    eapply frozen_caller_snapshots_independently_separated_after_safe_field_update;
      eauto.
    + exact (proj1 (proj2 (proj2 Hfrozen))).
    + exact (proj1 (proj2 (proj2 (proj2 Hfrozen)))).
    + exact (proj1 (proj2 (proj2 (proj2 (proj2 (proj2 Hfrozen)))))).
    + eapply executing_authority_colors_runtime_mutable.
      * exact Hwf.
...
Lemma principled_root_scoped_frozen_mutable_rdm_after_field_write_reflection :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h',
    principled_root_scoped_frozen_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h' [Hfrozen [Hcomponents Horigins]] Htyping
    Hscope Heval Hruntimes Hmutable Hretained Howned.
  assert (HsGamma : sGamma' = sGamma) by
    (inversion Htyping; reflexivity).
  assert (HrGamma : rGamma' = rGamma) by
    (inversion Heval; reflexivity).
  subst sGamma' rGamma'.
  have Hfull_post :=
    principled_frozen_authority_history_after_field_write_reflection CT P Z
      cutoff authority sGamma mt rGamma h stack incoming snapshots x field y
      sGamma rGamma h' Hfrozen Htyping Hscope Heval Hruntimes Hmutable
      Hretained Howned.
  have Hlive_post := principled_live_mutable_rdm_history_after_field_write CT
    P Z cutoff authority sGamma mt rGamma h stack incoming x field y sGamma
    rGamma h' (conj (proj1 Hfrozen) Hcomponents) Htyping Hscope Heval.
  split; [exact Hfull_post|]. split; [exact (proj2 Hlive_post)|].
  eapply frozen_caller_snapshots_active_resume_origins_after_graph_reflection;
    eauto.
  exact (proj1 (proj2 (proj2 (proj2 Hfrozen)))).
Qed.

Lemma principled_root_scoped_frozen_mutable_rdm_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h',
    principled_root_scoped_frozen_mutable_rdm_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h' Hstate Htyping Hscope Heval.
  destruct Hstate as [Hfrozen [Hcomponents Horigins]].
  have Hmain := proj1 Hfrozen.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hmain))))).
  have Heffect := typed_field_write_component_effect CT authority sGamma mt
  ...
  destruct Heffect as
    [[Hruntimes [Hmutable [Hretained Howned]]] |
     [lx [old [written [Hheap [Hobj Hendpoints]]]]]].
  - eapply principled_root_scoped_frozen_mutable_rdm_after_field_write_reflection.
    + exact (conj Hfrozen (conj Hcomponents Horigins)).
    + exact Htyping.
    + exact Hscope.
    + exact Heval.
    + exact Hruntimes.
  ...
  - subst h'.
    have Hfull_post := principled_frozen_authority_history_after_field_write CT
      P Z cutoff authority sGamma mt rGamma h stack incoming snapshots x field
      y sGamma rGamma (update_field h lx field (Iot written)) Hfrozen Htyping
      Hscope Heval.
    have Hlive_post := principled_live_mutable_rdm_history_after_field_write CT
      P Z cutoff authority sGamma mt rGamma h stack incoming x field y sGamma
      rGamma (update_field h lx field (Iot written))
      (conj Hmain Hcomponents) Htyping Hscope Heval.
    split; [exact Hfull_post|]. split; [exact (proj2 Hlive_post)|].
    eapply frozen_caller_snapshots_active_resume_origins_after_safe_field_update;
      eauto.
    + exact (proj1 (proj2 (proj2 Hfrozen))).
    + exact (proj1 (proj2 (proj2 (proj2 Hfrozen)))).
    + eapply executing_authority_colors_runtime_mutable.
...
Lemma allocation_flow_normal_form_old_prefix :
  forall CT h active stack qc source middle target,
    authority_flow_connected CT h active stack source middle ->
    allocation_flow_normal_form CT h active stack qc middle target ->
    allocation_flow_normal_form CT h active stack qc source target.
Proof.
  intros CT h active stack qc source middle target Hprefix Hnormal.
  destruct Hnormal as
    [target Hold
    | target anchor Hcapability Hconnected
    | anchor Hqc Hroot Hconnected].
  - apply allocation_flow_old. eapply rt_trans; eauto.
  - eapply allocation_flow_hits_capability; [exact Hcapability|].
    eapply rt_trans; eauto.
  - eapply allocation_flow_neutral_fresh; [exact Hqc|exact Hroot|].
    eapply rt_trans; eauto.
Qed.
...
Lemma authority_new_attachment_fresh :
  forall CT h active stack qc,
    authority_new_attachment CT h active stack qc (dom h).
Proof.
  intros. exists (dom h). split; [left; reflexivity|apply rt_refl].
Qed.
...
Lemma authority_new_attachment_transport :
  forall CT h active stack qc first second,
    authority_new_attachment CT h active stack qc first ->
    authority_color_connected CT h active stack first second ->
    authority_new_attachment CT h active stack qc second.
Proof.
  intros CT h active stack qc first second
    [anchor [Hanchor Hanchor_first]] Hfirst_second.
  exists anchor. split; [exact Hanchor|].
...
Lemma authority_new_entry_transport :
  forall CT h active stack qc first second,
    authority_color_connected CT h active stack first second ->
    authority_new_entry CT h active stack qc second ->
    authority_new_entry CT h active stack qc first.
Proof.
  intros CT h active stack qc first second Hfirst_second
    [anchor [Hanchor Hsecond_anchor]].
  exists anchor. split; [exact Hanchor|].
...
Lemma fresh_retained_edge_target_is_authority_attachment :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    authority stack target,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
...
Proof.
  ...
  - assert (HfdC : sf_def_rel CT C field fdef).
    ...
    have Hroot := new_creation_rdm_field_target_has_creation_root
      CT sGamma mt rGamma h x qc C args sGamma' vals field fdef target
      Hwf Htyping Hvals Hfield HfdC Hrdm.
    eapply authority_new_attachment_typed_root; exact Hroot.
  - assert (HfdC : sf_def_rel CT C field fdef).
    { eapply field_inheritance_subtyping; eauto. }
    have Hroot := new_creation_mut_field_target_has_mut_root
      CT sGamma mt rGamma h x qc C args sGamma' vals field fdef target
      Hwf Htyping Hvals Hfield HfdC Hmut.
    eapply authority_new_attachment_mut_root; exact Hroot.
Qed.

Lemma fresh_mutable_edge_target_is_authority_entry :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    authority stack target,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
...
Proof.
  ...
  - assert (HdefC : sf_def_rel CT C field fieldT).
    ...
    have Hroot := new_creation_rdm_field_target_has_creation_root
      ...
      Hwf Htyping Hvals Hfield HdefC Hrdm.
    eapply authority_new_entry_typed_root; exact Hroot.
Qed.
...
Lemma mutable_reachable_from_old_after_append :
  forall CT h newobj root target,
    wf_heap CT h ->
    root < dom h ->
    mutable_reachable CT (h ++ [newobj]) root target ->
    target < dom h /\ mutable_reachable CT h root target.
Proof.
  intros CT h newobj root target Hheap Hroot Hreachable.
  induction Hreachable as [root|root middle target Hprefix IH Hedge].
  - split; [exact Hroot|constructor].
  - destruct (IH Hroot) as [Hmiddle_dom Hold_prefix].
    destruct (mutable_edge_after_append CT h newobj middle target Hedge) as
      [Hold_edge | [Hmiddle_fresh Hnew]].
    + split.
      * eapply mutable_edge_target_dom; eauto.
...
Lemma fresh_mutable_reachable_has_creation_ancestor :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime target,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    runtime_lookup_list rGamma args = Some vals ->
    mutable_reachable CT
      (h ++ [mkObj (mkruntime_type qruntime C) vals]) (dom h) target ->
    target = dom h \/
    exists old_root,
      typed_root (qc2q qc) sGamma rGamma old_root /\
      mutable_reachable CT h old_root target.
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime target
    Hwf Htyping Hvals Hreachable.
  remember (dom h) as fresh eqn:Hfresh in Hreachable |- *.
  induction Hreachable as [fresh|fresh middle target Hprefix IH Hedge].
  - left. reflexivity.
  - destruct (IH Hfresh) as [Hmiddle_fresh |
      [old_root [Hold_root Hold_prefix]]].
    + subst middle.
      subst fresh.
      right. exists target. split.
      * eapply fresh_mutable_edge_target_has_creation_root; eauto.
      * constructor.
    + destruct (mutable_edge_after_append CT h
        (mkObj (mkruntime_type qruntime C) vals) middle target Hedge) as
        [Hold_edge | [Hmiddle_fresh Hnew]].
      * right. exists old_root. split; [exact Hold_root|].
        eapply mr_step; eauto.
      * have Hold_root_dom : old_root < dom h.
        { destruct Hold_root as
            [variable [T [Htype [Hvalue Hqualifier]]]].
          eapply wf_config_value_dom; eauto. }
        have Hmiddle_dom := mutable_reachable_target_dom CT h old_root middle
          (proj1 (proj2 Hwf)) Hold_root_dom Hold_prefix.
        subst middle. lia.
Qed.

Lemma authority_color_adjacent_after_new :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    authority stack left right,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    runtime_lookup_list rGamma args = Some vals ->
    authority_color_adjacent CT
...
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    authority stack left right Hwf Htyping Hvals [Hheap | Hframe].
  - destruct Hheap as [Hforward | Hbackward].
    + destruct (retained_edge_after_append CT h
      ...
      * subst left. right. split.
        -- apply authority_new_entry_fresh.
        -- eapply fresh_retained_edge_target_is_authority_attachment; eauto.
    + destruct (mutable_edge_after_append CT h
      ...
      * subst right. right. split.
        -- eapply fresh_mutable_edge_target_is_authority_entry; eauto.
        -- apply authority_new_attachment_fresh.
  - destruct Hframe as [frame [Hlive [Hleft Hright]]].
    ...
    + destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C
      ...
      * subst qc. right. split.
        -- apply authority_new_entry_typed_root. exact Hleft_old.
        -- subst right. apply authority_new_attachment_fresh.
      * subst qc. right. split.
        -- subst left. apply authority_new_entry_fresh.
        -- apply authority_new_attachment_typed_root. exact Hright_old.
      * right. split.
        -- subst left. apply authority_new_entry_fresh.
        -- subst right. apply authority_new_attachment_fresh.
...
Lemma allocation_flow_normal_form_old_step :
  forall CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    ...
    authority_flow_connected CT h
      (mk_watched_frame authority sGamma rGamma) stack source current ->
    authority_flow_step CT
      (h ++ [mkObj (mkruntime_type qruntime C) vals])
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) stack current target ->
...
Proof.
  intros CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    ...
    Hsource_runtime Hold Hstep.
  have Hcurrent_dom := authority_flow_connected_right_dom_from_left CT h
    (mk_watched_frame authority sGamma rGamma) stack source current Hframes
    Hold Hsource_dom.
  have Hcurrent_runtime :=
    authority_flow_connected_preserves_runtime_mutability CT h
      (mk_watched_frame authority sGamma rGamma) stack source current Mut_r
      Hframes (proj1 (proj2 Hwf)) Hold Hsource_runtime.
  destruct (authority_flow_step_after_new_from_old CT authority sGamma mt
    rGamma h stack x qc C args sGamma' vals qruntime current target Hwf
    Hframes Htyping Hvals Hcurrent_dom Hcurrent_runtime Hstep) as
    [Hreflects | [[Htarget [Hqc Hroot]] | Hcapability]].
  - apply allocation_flow_old. eapply rt_trans; eauto.
  - subst target. destruct current as [mode location].
    simpl in Hroot, Hold. destruct mode.
    + eapply allocation_flow_neutral_fresh
        with (anchor := location); [exact Hqc|exact Hroot|].
      eapply rt_trans; [exact Hold|].
      apply rt_step. apply authority_flow_forget.
    + inversion Hstep.
    + eapply allocation_flow_neutral_fresh
        with (anchor := location); assumption.
  - destruct current as [mode location].
    simpl in Hcapability, Hold. destruct mode.
    + eapply allocation_flow_hits_capability
        with (anchor := location); assumption.
    + inversion Hstep.
    + eapply allocation_flow_hits_capability
        with (anchor := location); [exact Hcapability|].
      eapply rt_trans; [exact Hold|].
      apply rt_step. apply authority_flow_promote.
      exists location. split.
      * left. exact Hcapability.
      * constructor.
Qed.
...
Lemma mutable_reachable_before_append_included :
  forall CT h new_object source target,
    mutable_reachable CT h source target ->
    mutable_reachable CT (h ++ [new_object]) source target.
Proof.
  intros CT h new_object source target Hreachable.
  induction Hreachable.
  - constructor.
  - eapply mr_step; [exact IHHreachable|].
    eapply mutable_edge_before_append_included; eauto.
Qed.

Lemma active_mutable_rdm_components_have_entry_origin_after_new :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    authority,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    runtime_lookup_list rGamma args = Some vals ->
    active_mutable_rdm_components_have_entry_origin CT h
      (mk_watched_frame authority sGamma rGamma)
      (h ++ [mkObj (mkruntime_type qruntime C) vals])
      (mk_watched_frame authority sGamma'
...
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    authority Hwf Htyping Hvals root target Hroot Hruntime Hreachable.
  destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C args
    sGamma' root Hwf Htyping Hroot) as
    [Hold_root | [Hroot_fresh Hrdm_creation]].
  - right. exists root. split; [exact Hold_root|exact Hreachable].
  - subst root qc. simpl in Hroot.
    destruct (fresh_mutable_reachable_has_creation_ancestor CT sGamma mt
      rGamma h x RDM_c C args sGamma' vals qruntime target Hwf Htyping
      Hvals Hreachable) as
      [Htarget_fresh | [entry_root [Hentry_root Hentry_reachable]]].
    + subst target. left. lia.
    + right. exists entry_root. split; [exact Hentry_root|].
      eapply mutable_reachable_before_append_included; eauto.
Qed.

Lemma retained_edge_before_append_included :
  forall CT h newobj left right,
    retained_mut_edge CT h left right ->
    retained_mut_edge CT (h ++ [newobj]) left right.
Proof.
  intros CT h newobj left right Hedge.
  inversion Hedge as
    [source target Hrdm |
     source target object field declaring field_def Hobject Hsource_mut
       Hfield Hbase Hdefinition Hmut]; subst.
  - constructor. eapply mutable_edge_before_append_included; eauto.
  - have Hsource_dom := Hobject.
    apply runtime_getObj_dom in Hsource_dom.
...
Lemma potential_connected_before_append_included :
  forall CT h newobj active stack left right,
    live_frames_wf CT h active stack ->
    potential_connected CT h active stack left right ->
    potential_connected CT (h ++ [newobj]) active stack left right.
Proof.
  intros CT h newobj active stack left right Hframes Hconnected.
  eapply potential_connected_map_edges; [|exact Hconnected].
  intros edge_left edge_right Hedge.
  have Hleft_dom :=
    potential_adjacent_left_dom CT h active stack edge_left edge_right
      Hframes Hedge.
  have Hright_dom :=
    potential_adjacent_right_dom CT h active stack edge_left edge_right
      Hframes Hedge.
  apply rt_step.
  destruct Hedge as [Hheap | [Hframe | Hreturn]].
  - left. destruct Hheap as [Hforward | Hbackward].
    + left. eapply retained_edge_before_append_included; eauto.
    + right. eapply mutable_edge_before_append_included; eauto.
  ...
  - right. right.
    ...
    split; [exact Hcallee_return|].
    split; [|exact Hroots].
    rewrite (r_muttype_app_preserve_old h newobj edge_left Hleft_dom).
    rewrite (r_muttype_app_preserve_old h newobj edge_right Hright_dom).
    exact Hruntime.
Qed.
...
Lemma enter_frozen_caller_snapshots_before_boundaries :
  forall CT h caller callee caller_colors snapshots stack boundary,
    ...
    frozen_caller_snapshots_before_boundaries
      ...
      (boundary :: stack).
Proof.
  intros CT h caller callee caller_colors snapshots stack boundary Hcutoff
    Hruntime Hcaller_wf Hbefore.
  constructor.
  - simpl. split.
  ...
  - eapply advance_frozen_caller_snapshots_before_boundaries. exact Hbefore.
Qed.

Lemma enter_nested_frozen_caller_snapshots_before_boundaries :
  forall CT h caller callee caller_colors snapshots stack boundary,
    boundary.(boundary_entry_cutoff) = dom h ->
    authority_colors_runtime_mutable h caller_colors ->
    wf_r_config CT caller.(frame_senv) caller.(frame_renv) h ->
...
Lemma frozen_allocation_step_preserves_covered :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime authority
    ...
    frozen_allocation_state_covered CT h
      (mk_watched_frame authority sGamma rGamma) colors qc source ->
    frozen_caller_authority_step CT
      (h ++ [mkObj (mkruntime_type qruntime C) vals])
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) source target ->
    frozen_allocation_state_covered CT h
      (mk_watched_frame authority sGamma rGamma) colors qc target.
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime authority
    colors source target Hwf Htyping Hvals Hruntime Hclosed Hsource Hstep
    Htarget_mode.
  inversion Hstep; subst; simpl in *.
  - destruct (Hsource (or_introl eq_refl)) as
      [[old_mode [Hold_mode Hold_color]] | Hcreation];
      [|right; exact Hcreation].
    destruct (retained_edge_after_append CT h
      (mkObj (mkruntime_type qruntime C) vals) left right H) as
      [Hold_edge | [Hleft_fresh Hnew]].
    + left. exists old_mode. split; [exact Hold_mode|].
      eapply frozen_caller_color_dangerous_retained; eauto.
    + subst left. exfalso.
      have Hdom := r_muttype_some_dom h (dom h) Mut_r
        (Hruntime old_mode (dom h) Hold_color). lia.
  ...
  - destruct (Hsource (or_intror eq_refl)) as
    ...
    + subst right. right. exists old_mode, left. repeat split; try assumption.
      eapply fresh_mutable_edge_target_has_creation_root with
        (qruntime := qruntime); eauto.
  - destruct (Hsource (or_introl eq_refl)) as
      [[old_mode [Hold_mode Hold_color]] | Hcreation];
      [|right; exact Hcreation].
    destruct (mutable_edge_after_append CT h
  ...
  - destruct (Hsource (or_introl eq_refl)) as
      ...
      [|right; exact Hcreation].
    destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C args
      sGamma' left Hwf Htyping H) as
      [Hleft_old | [Hleft_fresh Hleft_qc]];
    destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C args
      sGamma' right Hwf Htyping H0) as
      [Hright_old | [Hright_fresh Hright_qc]].
    + left. exists FlowProspective. split; [right; reflexivity|].
      eapply frozen_caller_color_dangerous_frame_join; eauto.
    + subst right qc. right. exists old_mode, left.
...
Lemma frozen_allocation_connected_preserves_covered :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime authority
    colors source target,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
...
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime authority
    colors source target Hwf Htyping Hvals Hruntime Hclosed Hsource Hconnected.
  induction Hconnected.
...
(** A dangerous frozen path that reaches the one fresh allocation must have
    entered it through an old constructor argument.  The argument remains
    both snapshot-colored and statically typed at the creation qualifier. *)
Lemma frozen_allocation_color_at_fresh_has_creation_root :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qruntime authority
    colors mode,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
...
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime authority
    colors mode Hwf Htyping Hvals Hruntime Hclosed Hmode
    [source [Hsource Hconnected]].
  assert (Hsource_covered : frozen_allocation_state_covered CT h
      (mk_watched_frame authority sGamma rGamma) colors qc source).
  ...
  have Hcovered := frozen_allocation_connected_preserves_covered CT sGamma mt
    rGamma h x qc C args sGamma' vals qruntime authority colors source
    (mode, dom h) Hwf Htyping Hvals Hruntime Hclosed Hsource_covered
    Hconnected Hmode.
  destruct Hcovered as
    [[old_mode [Hold_mode Hold_color]] |
     [root_mode [root [Hroot_mode [Hroot_color Hroot]]]]].
  - have Hfresh_runtime := Hruntime old_mode (dom h) Hold_color.
    have Hdom := r_muttype_some_dom h (dom h) Mut_r Hfresh_runtime. lia.
  - exists root_mode, root. repeat split; assumption.
Qed.
...
Lemma frozen_caller_snapshots_active_resume_origins_after_new :
  forall CT sGamma mt rGamma h x qc C args sGamma' vals qreceiver qruntime
    authority snapshots,
    wf_r_config CT sGamma rGamma h ->
    wf_r_config CT sGamma'
...
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qreceiver qruntime
    authority snapshots Hwf Hpost_wf Hsound Hpost_sound Htyping Hvals Hadapt
    Hruntime Hclosed Horigins new_snapshot snapshot_mode active_mode location
  ...
  have Hcovered := advance_frozen_caller_snapshot_after_new_covered CT sGamma
    ...
    snapshot_mode location Hwf Htyping Hvals
    (Hruntime old_snapshot Hold_snapshot)
    (Hclosed old_snapshot Hold_snapshot) Hsnapshot_mode Hsnapshot_color.
  destruct Hcovered as
  ...
  - destruct Htrigger as [Hactive_color | Hrdm].
    + have Hlocation_old : location < dom h.
      ...
      have Hempty_runtime : authority_colors_runtime_mutable h
          (Empty_set authority_flow_state).
      { intros mode root Hempty. inversion Hempty. }
      destruct (executing_authority_colors_after_new_covered CT sGamma mt
        ...
        Hempty_runtime Htyping Hvals Hadapt
        active_mode location Hactive_mode Hactive_color Hlocation_old) as
    ...
    + destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C args
      ...
      * subst location qc.
        have Hfresh_runtime :=
          Hruntime old_snapshot Hold_snapshot old_snapshot_mode (dom h)
            Hold_snapshot_color.
        have Hdom := r_muttype_some_dom h (dom h) Mut_r Hfresh_runtime. lia.
  - destruct qc; simpl in Hcreation_root.
    + have Howned : frame_owned_location CT h
      ...
      destruct (Horigins old_snapshot creation_mode FlowPowered creation_root
        Hold_snapshot Hcreation_mode (or_introl eq_refl) Hcreation_color
        (or_introl Hactive_color)) as
        [root_mode [root [Hroot_mode [Hroot_color Hroot]]]].
      exists root_mode, root. split; [exact Hroot_mode|]. split.
      * apply frozen_caller_authority_closure_contains. exact Hroot_color.
      * exact Hroot.
    + have Hmut := Hruntime old_snapshot Hold_snapshot creation_mode
        creation_root Hcreation_color.
    ...
    + destruct (Horigins old_snapshot creation_mode active_mode creation_root
        Hold_snapshot Hcreation_mode Hactive_mode Hcreation_color
        (or_intror Hcreation_root)) as
        [root_mode [root [Hroot_mode [Hroot_color Hroot]]]].
      exists root_mode, root. split; [exact Hroot_mode|]. split.
      * apply frozen_caller_authority_closure_contains. exact Hroot_color.
      * exact Hroot.
...
Lemma allocation_flow_normal_form_neutral_fresh_step :
  forall CT authority sGamma mt rGamma h stack x C args sGamma' vals
    qruntime source anchor target,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) stack ->
    stmt_typing CT sGamma mt (SNew x RDM_c C args) sGamma' ->
    runtime_lookup_list rGamma args = Some vals ->
    typed_root RDM sGamma rGamma anchor ->
    authority_flow_connected CT h
      (mk_watched_frame authority sGamma rGamma) stack
      source (FlowNeutral, anchor) ->
    authority_flow_step CT
      ...
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) stack
      (FlowNeutral, dom h) target ->
    allocation_flow_normal_form CT h
      (mk_watched_frame authority sGamma rGamma) stack RDM_c source target.
Proof.
  intros CT authority sGamma mt rGamma h stack x C args sGamma' vals
    qruntime source anchor target Hwf Hframes Htyping Hvals Hanchor
    Hconnected Hstep.
  inversion Hstep as
    [left right Hedge
    | left right Hedge
    ...
    | left right Hedge
    | location
    | location Hcapability]; subst.
  - destruct (mutable_edge_after_append CT h
      (mkObj (mkruntime_type qruntime C) vals) (dom h) right Hedge) as
      [Hold | [Hfresh Hnew]].
    + inversion Hold as [? ? oldobject ? ? ? Hobject].
      apply runtime_getObj_dom in Hobject. lia.
    + have Hright := fresh_mutable_edge_target_has_creation_root CT sGamma
        mt rGamma h x RDM_c C args sGamma' vals qruntime right Hwf Htyping
        Hvals Hedge.
      apply allocation_flow_old.
  ...
  - destruct (mutable_edge_after_append CT h
      ...
      [Hold | [Hfresh [field [D [fieldT
        [Hfield [Hbase [Hdef Hrdm]]]]]]]].
    + have Htarget_dom := mutable_edge_target_dom CT h right (dom h)
        (proj1 (proj2 Hwf)) Hold.
      lia.
    + subst right.
      have Htarget_dom := allocated_argument_location_is_old CT sGamma
        rGamma h args vals field (dom h) Hwf Hvals Hfield.
      lia.
  - destruct Hedge as [frame [Hlive [Hleft Hright]]].
    inversion Hlive; subst.
    + destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x RDM_c C
        args sGamma' (dom h) Hwf Htyping Hleft) as
        [Hleft_old | [Hleft_fresh Hleft_qc]].
      * destruct Hleft_old as
          [variable [T [Htype [Hvalue Hrdm]]]].
        have Hdom := wf_config_value_dom CT sGamma rGamma h variable
          (dom h) Hwf Hvalue. lia.
      * destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x RDM_c
          C args sGamma' right Hwf Htyping Hright) as
          [Hright_old | [Hright_fresh Hright_qc]].
        -- apply allocation_flow_old.
           eapply rt_trans; [exact Hconnected|].
           apply rt_step. apply authority_flow_neutral_frame.
           exists (mk_watched_frame authority sGamma rGamma).
           split; [constructor|]. split; assumption.
        -- subst right. eapply allocation_flow_neutral_fresh
             with (anchor := anchor); assumption.
    + have Hcaller_wf := live_frame_member_wf CT h
        (mk_watched_frame authority sGamma rGamma) stack
        boundary.(boundary_caller) Hframes.
      assert (Hcaller_live : live_frame_member
      ...
      specialize (Hcaller_wf Hcaller_live).
      destruct Hleft as [variable [T [Htype [Hvalue Hrdm]]]].
      have Hdom := wf_config_value_dom CT
        boundary.(boundary_caller).(frame_senv)
        boundary.(boundary_caller).(frame_renv) h variable (dom h)
        Hcaller_wf Hvalue.
      lia.
  - have Hauthority := fresh_live_after_rdm_new_implies_mut_authority CT
      sGamma mt rGamma h x C args sGamma' vals qruntime authority stack Hwf
      Hframes Htyping Hvals Hcapability.
    subst authority.
    eapply allocation_flow_hits_capability with (anchor := anchor).
    + destruct Hanchor as [variable [T [Htype [Hvalue Hrdm]]]].
      exists variable, T. repeat split; try assumption.
      unfold capability_in_context. right. split; [exact Hrdm|reflexivity].
    + eapply rt_trans; [exact Hconnected|].
      apply rt_step. apply authority_flow_promote.
      eapply typed_rdm_root_is_live_under_mut_authority. exact Hanchor.
Qed.

Lemma allocation_flow_normal_form_step :
  forall CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime source current target,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
...
Proof.
  intros CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime source current target Hwf Hframes Htyping Hvals Hsource_dom
    Hsource_runtime Hnormal Hstep.
  destruct Hnormal as
    [current Hold
    | current capability Hcapability Hreaches
    | anchor Hqc Hanchor Hreaches].
  - eapply allocation_flow_normal_form_old_step; eauto.
  - eapply allocation_flow_hits_capability
...
Lemma pending_call_stateful_authority_after_new :
  forall CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime tracked_depth,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) stack ->
    ...
    pending_call_stateful_authority_separated CT h
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    runtime_lookup_list rGamma args = Some vals ->
    pending_call_stateful_authority_separated CT
      (h ++ [mkObj (mkruntime_type qruntime C) vals])
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) stack tracked_depth.
Proof.
  intros CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime tracked_depth Hwf Hframes Hsound Hpending Htyping Hvals
    boundary above below capability owned Hpartition Htracked Hchannel_free
    Howned Hcapability Hreachable.
  have Hold_partition :
      live_call_partition (mk_watched_frame authority sGamma rGamma) stack
        boundary above below.
  { eapply live_call_partition_change_active. exact Hpartition. }
  have Hcaller_frames :=
    live_call_partition_caller_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) stack boundary above below
      Hold_partition Hframes.
  have Hcaller_sound :=
    live_call_partition_caller_frames_authority_sound h
      (mk_watched_frame authority sGamma rGamma) stack boundary above below
      Hold_partition Hsound.
  have Hcapability_old :
      In Loc
        (live_capability_set CT h boundary.(boundary_caller) below)
        capability.
  { eapply live_capability_before_append_included; eauto. }
  have Hsource_dom :=
    live_capability_location_dom CT h boundary.(boundary_caller) below
      capability Hcaller_frames Hcapability_old.
  have Hsource_runtime :=
    live_capability_members_runtime_mutable CT h boundary.(boundary_caller)
      below Hcaller_frames Hcaller_sound capability Hcapability_old.
  have Hnormal := allocation_flow_normal_form_connected CT authority sGamma
    mt rGamma h stack x qc C args sGamma' vals qruntime
    (FlowPowered, capability) (FlowPowered, capability)
    (FlowPowered, owned) Hwf Hframes Htyping Hvals Hsource_dom
    Hsource_runtime
    (allocation_flow_normal_form_refl CT h
      (mk_watched_frame authority sGamma rGamma) stack qc
      (FlowPowered, capability))
    Hreachable.
  inversion Hnormal as
    [target Hold_path
    | target anchor Hanchor Hanchor_path
    | anchor Hqc Hanchor Hanchor_path]; subst.
  - have Howned_dom := authority_flow_connected_right_dom_from_left CT h
      (mk_watched_frame authority sGamma rGamma) stack
      (FlowPowered, capability) (FlowPowered, owned) Hframes Hold_path
      Hsource_dom.
    have Habove_frames :=
      live_call_partition_above_frames_wf CT h
        (mk_watched_frame authority sGamma rGamma) stack boundary above below
        Hold_partition Hframes.
    have Howned_old : In Loc
        (pending_owned_authority_set CT h
          (mk_watched_frame authority sGamma rGamma) above) owned.
    { destruct (new_live_reachability_after_new_has_origin CT
        authority sGamma mt rGamma h above x qc C args sGamma' vals
        qruntime owned Hwf Habove_frames Htyping Hvals Howned) as
        [Hfresh | Howned_old].
      - subst owned. simpl in Howned_dom. lia.
      - exact Howned_old. }
    apply (Hpending boundary above below capability owned Hold_partition
      Htracked Hchannel_free Howned_old Hcapability_old).
    exact Hold_path.
  - have Hanchor_owned :
      In Loc
        (pending_owned_authority_set CT h
          (mk_watched_frame authority sGamma rGamma) above) anchor.
    { exists anchor. split.
      - left. exact Hanchor.
      - constructor. }
    apply (Hpending boundary above below capability anchor Hold_partition
      Htracked Hchannel_free Hanchor_owned Hcapability_old).
    exact Hanchor_path.
Qed.

Lemma new_live_cone_to_old_mutable_has_old_origin :
  forall CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime capability target,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) stack ->
    live_frames_wf CT
    ...
    live_frames_authority_sound
      ...
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) stack ->
    live_stack_authorities_chain authority stack ->
    (qc = RDM_c -> exists receiver,
      runtime_getVal rGamma 0 = Some (Iot receiver) /\
      r_muttype h receiver = Some qruntime) ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    ...
    In Loc
      (live_capability_set CT
        (h ++ [mkObj (mkruntime_type qruntime C) vals])
        (mk_watched_frame authority sGamma'
          (update_r_env_value rGamma x (Iot (dom h)))) stack) capability ->
    potential_connected CT
      (h ++ [mkObj (mkruntime_type qruntime C) vals])
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) stack
    ...
    exists origin,
      In Loc
        (live_capability_set CT h
          (mk_watched_frame authority sGamma rGamma) stack) origin /\
      potential_connected CT h
        (mk_watched_frame authority sGamma rGamma) stack origin target.
Proof.
  intros CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime capability target Hwf Hframes Hpost_frames Hpost_sound Hchain
    Hrdm_runtime Htyping Hvals Htarget_dom Htarget_runtime Hcapability
    Hconnected.
  destruct (potential_connected_after_new CT sGamma mt rGamma h x qc C args
    sGamma' vals qruntime authority stack capability target Hwf Hframes
    Hrdm_runtime Htyping Hvals Hconnected) as
    [Hold_connected | [Hcapability_entry Htarget_attachment]].
  - have Hcapability_dom : capability < dom h.
    ...
    { eapply new_live_reachability_to_old_location_has_old_origin
        with (mt := mt) (x := x) (qc := qc) (C := C) (args := args)
          (sGamma' := sGamma') (vals := vals) (qruntime := qruntime);
        eauto. }
    exists capability. split; assumption.
  - destruct (potential_new_attachment_to_old_has_typed_anchor CT h
      (mk_watched_frame authority sGamma rGamma) stack qc target Hframes
      Htarget_dom Htarget_attachment) as
      [target_anchor [Htarget_anchor Hanchor_target]].
    have Hheap : wf_heap CT h := proj1 (proj2 Hwf).
    have Hanchor_runtime : r_muttype h target_anchor = Some Mut_r.
    { eapply potential_connected_reflects_runtime_mutability; eauto. }
    destruct qc.
    + destruct Htarget_anchor as
        [Htyped | [Hmut | [Himpossible Hcaller]]];
        [| |discriminate].
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Htyped.
        -- exact Hanchor_target.
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Hmut.
        -- exact Hanchor_target.
    + destruct Htarget_anchor as
        [Himm | [Hmut | [Himpossible Hcaller]]].
      * have Himmutable := typed_imm_root_runtime_immutable CT sGamma rGamma h
          target_anchor Hwf Himm.
        congruence.
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Hmut.
        -- exact Hanchor_target.
      * discriminate.
    + simpl in Htarget_anchor.
      assert (Htarget_case :
        (typed_root RDM sGamma rGamma target_anchor \/
         immediate_rdm_caller_root h
           (mk_watched_frame authority sGamma rGamma) stack target_anchor) \/
        In Loc
          (live_capability_set CT h
            (mk_watched_frame authority sGamma rGamma) stack)
          target_anchor).
      { destruct Htarget_anchor as
          [Hactive | [Hmut | [Hqc Hcaller]]].
        - left. left. exact Hactive.
        - right. eapply typed_mut_root_is_live_capability. exact Hmut.
        - left. right. exact Hcaller. }
      destruct Htarget_case as [Htarget_creation | Htarget_capability].
      * destruct Hcapability_entry as
          [capability_anchor
            [[Hanchor_fresh | [Hanchor_active |
              [Hanchor_qc Hanchor_caller]]] Hcapability_anchor]].
        -- subst capability_anchor.
           have Hcapability_fresh :=
             potential_connected_to_fresh_is_fresh CT h
               (mk_watched_frame authority sGamma rGamma) stack capability
               Hframes Hcapability_anchor.
           subst capability.
           have Hauthority_mut :=
             fresh_live_after_rdm_new_implies_mut_authority CT sGamma mt
               rGamma h x C args sGamma' vals qruntime authority stack Hwf
               Hframes Htyping Hvals Hcapability.
           subst authority.
           exists target_anchor. split.
           ++ destruct Htarget_creation as [Hactive | Hcaller].
              ** eapply typed_rdm_root_is_live_under_mut_authority.
                 exact Hactive.
              ** eapply immediate_rdm_caller_root_live_under_mut_authority;
                   eauto.
           ++ exact Hanchor_target.
        -- have Hanchor_dom : capability_anchor < dom h.
           { destruct Hanchor_active as
               [variable [T [Htype [Hvalue Hrdm]]]].
             eapply wf_config_value_dom; eauto. }
           have Hcapability_dom : capability < dom h.
           { eapply potential_connected_left_dom_from_right; eauto. }
           ...
           { eapply new_live_reachability_to_old_location_has_old_origin
               with (mt := mt) (x := x) (qc := RDM_c) (C := C)
                 ...
                 (qruntime := qruntime); eauto. }
           exists capability. split; [exact Hcapability_old|].
           eapply rt_trans; [exact Hcapability_anchor|].
           eapply rt_trans.
           ++ eapply rdm_creation_anchors_potentially_connected.
              ** left. exact Hanchor_active.
              ** exact Htarget_creation.
           ++ exact Hanchor_target.
        -- have Hanchor_dom : capability_anchor < dom h.
           ...
           eapply rt_trans.
           ++ eapply rdm_creation_anchors_potentially_connected.
              ** right. exact Hanchor_caller.
              ** exact Htarget_creation.
           ++ exact Hanchor_target.
      * exists target_anchor. split; assumption.
Qed.

Lemma authority_new_live_cone_to_old_mutable_has_old_origin :
  forall CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime capability target,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
...
Proof.
  intros CT authority sGamma mt rGamma h stack x qc C args sGamma' vals
    qruntime capability target Hwf Hframes Hpost_frames Hpost_sound
    Hrdm_runtime Htyping Hvals Htarget_dom Htarget_runtime Hcapability
    Hconnected.
  destruct (authority_color_connected_after_new CT sGamma mt rGamma h x qc
    C args sGamma' vals qruntime authority stack capability target Hwf
    Htyping Hvals Hconnected) as
    [Hold_connected | [Hcapability_entry Htarget_attachment]].
  - have Hcapability_dom : capability < dom h.
    { eapply potential_connected_left_dom_from_right.
      - exact Hframes.
      - eapply authority_color_connected_is_potential_connected.
        exact Hold_connected.
      - exact Htarget_dom. }
    have Hcapability_old :
      In Loc
        (live_capability_set CT h
    ...
    { eapply new_live_reachability_to_old_location_has_old_origin
        ...
        eauto. }
    exists capability. split; assumption.
  - destruct (authority_new_attachment_to_old_has_typed_anchor CT h
      (mk_watched_frame authority sGamma rGamma) stack qc target Hframes
      Htarget_dom Htarget_attachment) as
      [target_anchor [Htarget_anchor Hanchor_target]].
    have Hheap : wf_heap CT h := proj1 (proj2 Hwf).
    destruct qc.
    + destruct Htarget_anchor as [Htyped | Hmut].
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Htyped.
        -- exact Hanchor_target.
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Hmut.
        -- exact Hanchor_target.
    + destruct Htarget_anchor as [Himm | Hmut].
      * have Himmutable := typed_imm_root_runtime_immutable CT sGamma rGamma
          h target_anchor Hwf Himm.
        have Htarget_immutable :=
          authority_color_connected_preserves_runtime_mutability CT h
            (mk_watched_frame authority sGamma rGamma) stack target_anchor
            target Imm_r Hframes Hheap Hanchor_target Himmutable.
        congruence.
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Hmut.
        -- exact Hanchor_target.
    + destruct Htarget_anchor as [Htarget_rdm | Htarget_mut].
      2: {
        exists target_anchor. split.
        - eapply typed_mut_root_is_live_capability. exact Htarget_mut.
        - exact Hanchor_target.
      }
      destruct Hcapability_entry as
        [capability_anchor [[Hanchor_fresh | Hanchor_rdm]
          Hcapability_anchor]].
      * subst capability_anchor.
        have Hcapability_fresh :=
          potential_connected_to_fresh_is_fresh CT h
            (mk_watched_frame authority sGamma rGamma) stack capability
            Hframes
            (authority_color_connected_is_potential_connected CT h
              (mk_watched_frame authority sGamma rGamma) stack capability
              (dom h) Hcapability_anchor).
        subst capability.
        have Hauthority_mut :=
          fresh_live_after_rdm_new_implies_mut_authority CT sGamma mt rGamma
            h x C args sGamma' vals qruntime authority stack Hwf Hframes
            Htyping Hvals Hcapability.
        subst authority.
        exists target_anchor. split.
        -- eapply typed_rdm_root_is_live_under_mut_authority.
           exact Htarget_rdm.
      ...
      * have Hanchor_dom : capability_anchor < dom h.
        ...
        -- apply rt_step. right.
           exists (mk_watched_frame authority sGamma rGamma).
           split; [constructor|].
           split; [exact Hanchor_rdm|exact Htarget_rdm].
        -- exact Hanchor_target.
Qed.

Lemma authority_new_owned_cone_to_old_mutable_has_old_origin :
  forall CT authority sGamma mt rGamma h owned_stack graph_stack x qc C args
    sGamma' vals qruntime capability target,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) owned_stack ->
    live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) graph_stack ->
    (qc = RDM_c -> exists receiver,
      runtime_getVal rGamma 0 = Some (Iot receiver) /\
      r_muttype h receiver = Some qruntime) ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    ...
    In Loc
      (live_capability_set CT
        (h ++ [mkObj (mkruntime_type qruntime C) vals])
        (mk_watched_frame authority sGamma'
          (update_r_env_value rGamma x (Iot (dom h)))) owned_stack)
      capability ->
    authority_color_connected CT
      (h ++ [mkObj (mkruntime_type qruntime C) vals])
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))) graph_stack
      capability target ->
    exists origin,
      In Loc
        (live_capability_set CT h
          (mk_watched_frame authority sGamma rGamma) owned_stack) origin /\
      authority_color_connected CT h
        (mk_watched_frame authority sGamma rGamma) graph_stack origin target.
Proof.
  intros CT authority sGamma mt rGamma h owned_stack graph_stack x qc C args
    sGamma' vals qruntime capability target Hwf Howned_frames Hgraph_frames
    Hrdm_runtime Htyping Hvals Htarget_dom Htarget_runtime Hcapability
    Hconnected.
  destruct (authority_color_connected_after_new CT sGamma mt rGamma h x qc
    C args sGamma' vals qruntime authority graph_stack capability target Hwf
    Htyping Hvals Hconnected) as
    [Hold_connected | [Hcapability_entry Htarget_attachment]].
  - have Hcapability_dom : capability < dom h.
    { eapply potential_connected_left_dom_from_right.
    ...
    have Hcapability_old :
      In Loc
        (live_capability_set CT h
          (mk_watched_frame authority sGamma rGamma) owned_stack) capability.
    { eapply new_live_reachability_to_old_location_has_old_origin
        with (mt := mt) (x := x) (qc := qc) (C := C) (args := args)
          (sGamma' := sGamma') (vals := vals) (qruntime := qruntime);
        eauto. }
    exists capability. split; assumption.
  - destruct (authority_new_attachment_to_old_has_typed_anchor CT h
      (mk_watched_frame authority sGamma rGamma) graph_stack qc target
      Hgraph_frames Htarget_dom Htarget_attachment) as
      [target_anchor [Htarget_anchor Hanchor_target]].
    have Hheap : wf_heap CT h := proj1 (proj2 Hwf).
    destruct qc.
    + destruct Htarget_anchor as [Htyped | Hmut].
    ...
    + destruct Htarget_anchor as [Himm | Hmut].
      * have Himmutable := typed_imm_root_runtime_immutable CT sGamma rGamma
        ...
        have Htarget_immutable :=
          authority_color_connected_preserves_runtime_mutability CT h
            (mk_watched_frame authority sGamma rGamma) graph_stack
            target_anchor target Imm_r Hgraph_frames Hheap Hanchor_target
            Himmutable.
        congruence.
      * exists target_anchor. split.
        -- eapply typed_mut_root_is_live_capability. exact Hmut.
    ...
    + destruct Htarget_anchor as [Htarget_rdm | Htarget_mut].
      ...
      * subst capability_anchor.
        have Hcapability_fresh :=
          potential_connected_to_fresh_is_fresh CT h
            (mk_watched_frame authority sGamma rGamma) graph_stack capability
            Hgraph_frames
            (authority_color_connected_is_potential_connected CT h
              (mk_watched_frame authority sGamma rGamma) graph_stack
              capability (dom h) Hcapability_anchor).
        subst capability.
        have Hauthority_mut :=
          fresh_live_after_rdm_new_implies_mut_authority CT sGamma mt rGamma
            h x C args sGamma' vals qruntime authority owned_stack Hwf
            Howned_frames Htyping Hvals Hcapability.
        subst authority.
        exists target_anchor. split.
        -- eapply typed_rdm_root_is_live_under_mut_authority.
           exact Htarget_rdm.
...
Lemma authority_color_history_after_new :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x qc C args
    sGamma' rGamma' h',
    authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x qc C args
    sGamma' rGamma' h' [Hlive [Hcolors Hcutoffs]] Htyping Heval.
  have Hcomponent : component_forward_history_state CT P Z
      (live_capability_set CT h
        (mk_watched_frame authority sGamma rGamma) stack)
      cutoff sGamma rGamma h.
  { eapply mutable_authority_component_history with (authority := authority).
    exact (proj1 Hlive). }
  have Hlive_post := live_history_after_new CT P Z cutoff authority sGamma mt
    rGamma h stack x qc C args sGamma' rGamma' h' Hlive Hcomponent Htyping
    Heval.
  ...
  - destruct Hlive as
    ...
    have Hprotected_old : protected < dom h.
    { have Hbound := Hzone_bound protected Hprotected. lia. }
    destruct (authority_color_connected_after_new CT sGamma mt rGamma h x
      qc C args sGamma' vals new_runtime authority stack capability protected
      Hwf Htyping Hargs Hconnected) as
      [Hold_connected |
    ...
    + have Hcapability_old : capability < dom h.
      { eapply potential_connected_left_dom_from_right.
        ...
        - eapply authority_color_connected_is_potential_connected.
          exact Hold_connected.
        - exact Hprotected_old. }
      have Hcapability_pre : In Loc
          (live_capability_set CT h
            (mk_watched_frame authority sGamma rGamma) stack) capability.
      { eapply new_live_reachability_to_old_location_has_old_origin
          with (mt := mt) (x := x) (qc := qc) (C := C) (args := args)
            (sGamma' := sGamma') (vals := vals)
            (qruntime := new_runtime); eauto. }
      exact (Hcolors capability protected Hcapability_pre Hprotected
        Hold_connected).
    + destruct (authority_new_attachment_to_old_has_typed_anchor CT h
        (mk_watched_frame authority sGamma rGamma) stack qc protected Hframes
        Hprotected_old Hprotected_attachment) as
      ...
      have Hprotected_runtime_post : r_muttype
          (h ++ [mkObj (mkruntime_type new_runtime C) vals]) protected =
          Some Mut_r.
      { eapply authority_color_connected_preserves_runtime_mutability; eauto.
        exact (proj1 (proj2 (proj1 Hpost_frames))). }
      have Hprotected_runtime : r_muttype h protected = Some Mut_r.
      { rewrite (r_muttype_app_preserve_old h
          (mkObj (mkruntime_type new_runtime C) vals) protected Hprotected_old)
        ...
        exact Hprotected_runtime_post. }
      destruct qc.
      * destruct Hzone_anchor as [Hzone_mut | Hzone_mut].
        -- have Hzone_capability : In Loc
              (live_capability_set CT h
                (mk_watched_frame authority sGamma rGamma) stack)
              zone_anchor.
           { eapply typed_mut_root_is_live_capability. exact Hzone_mut. }
        ...
        -- have Hzone_capability : In Loc
           ...
           exact (Hcolors zone_anchor protected Hzone_capability Hprotected
             Hzone_connected).
      * destruct Hzone_anchor as [Hzone_imm | Hzone_mut].
        -- have Hzone_immutable := typed_imm_root_runtime_immutable CT sGamma
             rGamma h zone_anchor Hwf Hzone_imm.
           have Hprotected_immutable :=
             authority_color_connected_preserves_runtime_mutability CT h
               (mk_watched_frame authority sGamma rGamma) stack
               zone_anchor protected Imm_r Hframes Hheap_wf Hzone_connected
               Hzone_immutable.
           congruence.
        -- have Hzone_capability : In Loc
              (live_capability_set CT h
           ...
           exact (Hcolors zone_anchor protected Hzone_capability Hprotected
             Hzone_connected).
      * destruct Hzone_anchor as [Hzone_rdm | Hzone_mut].
        2: {
          have Hzone_capability : In Loc
              (live_capability_set CT h
                (mk_watched_frame authority sGamma rGamma) stack)
        ...
        destruct Hcapability_entry as
          [capability_anchor
            [[Hanchor_fresh | Hanchor_rdm] Hcapability_anchor]].
        -- subst capability_anchor.
           have Hcapability_fresh :=
             potential_connected_to_fresh_is_fresh CT h
               (mk_watched_frame authority sGamma rGamma) stack capability
           ...
           have Hauthority_mut :=
             fresh_live_after_rdm_new_implies_mut_authority CT sGamma mt
               rGamma h x C args sGamma' vals new_runtime authority stack Hwf
               Hframes Htyping Hargs Hcapability.
           subst authority.
           have Hzone_capability : In Loc
               (live_capability_set CT h
                 (mk_watched_frame Mut_r sGamma rGamma) stack) zone_anchor.
        ...
        -- have Hanchor_dom : capability_anchor < dom h.
           ...
           { eapply new_live_reachability_to_old_location_has_old_origin
               with (mt := mt) (x := x) (qc := RDM_c) (C := C)
                 (args := args) (sGamma' := sGamma') (vals := vals)
                 (qruntime := new_runtime); eauto. }
           apply (Hcolors capability protected Hcapability_pre Hprotected).
           eapply authority_color_connected_trans.
           ++ exact Hcapability_anchor.
           ++ eapply authority_color_connected_trans.
              ** apply rt_step. right.
                 exists (mk_watched_frame authority sGamma rGamma).
                 split; [constructor|].
                 split; [exact Hanchor_rdm|exact Hzone_rdm].
              ** exact Hzone_connected.
  - eapply live_boundary_cutoffs_valid_heap_growth; [|exact Hcutoffs].
    eapply eval_stmt_preserves_heap_domain_simple; exact Heval.
Qed.

Lemma principled_authority_color_history_after_new :
  forall CT P Z cutoff authority sGamma mt rGamma h stack x qc C args
    sGamma' rGamma' h' tracked_depth,
    principled_authority_color_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack tracked_depth h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    eval_stmt CT rGamma h (SNew x qc C args) OK rGamma' h' ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x qc C args
    sGamma' rGamma' h' tracked_depth [Hstate Hpending] Htyping Heval.
  have Hpost := authority_color_history_after_new CT P Z cutoff authority
    sGamma mt rGamma h stack x qc C args sGamma' rGamma' h' Hstate Htyping
    Heval.
  split; [exact Hpost|].
  have Hlive := proj1 Hstate.
  have Hframes := proj1 (proj2 Hlive).
  have Hsound := proj1 (proj2 (proj2 Hlive)).
  ...
  { destruct rGamma. reflexivity. }
  rewrite Hupdate.
  eapply pending_call_stateful_authority_after_new; eauto.
Qed.
...
Lemma bounded_frozen_history_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack
    x expression old value,
    bounded_frozen_flexible_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SVarAss x expression) sGamma ->
    readonly_state_method_scope mt ->
    runtime_getVal rGamma x = Some old ->
    ...
    bounded_frozen_flexible_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma
        (update_r_env_value rGamma x value)) stack h.
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x expression old
    value [incoming [snapshots [Hstate Hbefore]]] Htyping Hscope Hvalue Heval.
  exists incoming,
    (advance_frozen_caller_snapshots CT h
      (mk_watched_frame authority sGamma
        (update_r_env_value rGamma x value)) snapshots).
  split.
  - eapply principled_frozen_authority_history_after_assignment; eauto.
  - eapply advance_frozen_caller_snapshots_before_boundaries. exact Hbefore.
Qed.

Lemma bounded_frozen_history_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma',
    bounded_frozen_flexible_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
    stmt_typing CT sGamma mt (SLocal T x) sGamma' ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack T x sGamma'
    [incoming [snapshots [Hstate Hbefore]]] Htyping Hnone.
  exists incoming,
    (advance_frozen_caller_snapshots CT h
      (mk_watched_frame authority sGamma'
        (set_vars rGamma (vars rGamma ++ [Null_a]))) snapshots).
  split.
  - eapply principled_frozen_authority_history_after_local; eauto.
  - eapply advance_frozen_caller_snapshots_before_boundaries. exact Hbefore.
Qed.

Lemma bounded_frozen_history_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack
    x field y sGamma' rGamma' h',
    bounded_frozen_flexible_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x field y
    sGamma' rGamma' h' [incoming [snapshots [Hstate Hbefore]]] Htyping
    Hscope Heval.
  exists incoming,
    (advance_frozen_caller_snapshots CT h'
      (mk_watched_frame authority sGamma' rGamma') snapshots).
  split.
  - eapply principled_frozen_authority_history_after_field_write; eauto.
  - eapply advance_frozen_caller_snapshots_before_boundaries. exact Hbefore.
Qed.

Lemma bounded_frozen_history_after_new :
  forall CT P Z cutoff authority sGamma mt rGamma h stack
    x qc C args sGamma' rGamma' h',
    bounded_frozen_flexible_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack x qc C args
    sGamma' rGamma' h' [incoming [snapshots [Hstate Hbefore]]] Htyping Heval.
  exists incoming,
    (advance_frozen_caller_snapshots CT h'
      (mk_watched_frame authority sGamma' rGamma') snapshots).
  split.
  - eapply principled_frozen_authority_history_after_new; eauto.
  - eapply advance_frozen_caller_snapshots_before_boundaries. exact Hbefore.
Qed.

(** Atomic cases for the explicit private induction.  Each transition keeps
    the incoming phase fixed, advances every existing snapshot through the
    new active state, and preserves its boundary-age certificate. *)
Lemma private_frozen_statement_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x expression old value,
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x expression old value
    [Hstate [Horigins [Hbefore [Hcovered [Hnested Hcompleted]]]]]
    Htyping Hscope Hvalue
    Heval.
  have Hmain := proj1 Hstate.
  have Hstate_parts := Hstate.
  destruct Hstate_parts as
    [_ [_ [_ [Hsnapshot_closed [_ [_ [_ [_ [Hexposure_wf _]]]]]]]]].
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hmain))))).
  have Hdescend := rdm_roots_descend_after_assignment CT sGamma mt rGamma h
    x expression old value Hwf Htyping Hscope Hvalue Heval.
  ...
  { intros location Hlocation.
    ...
    eapply assignment_live_reachability_is_old with
      ...
      (stack := []); eauto.
    apply frame_owned_location_iff_active_live. exact Hlocation. }
  have Hpost : principled_frozen_authority_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma
        (update_r_env_value rGamma x value)) stack incoming
      (advance_frozen_caller_snapshots CT h
        (mk_watched_frame authority sGamma
          (update_r_env_value rGamma x value)) snapshots) h by
    (eapply principled_frozen_authority_history_after_assignment; eauto).
  have Hpost_parts := Hpost.
  destruct Hpost_parts as
    [_ [_ [_ [_ [Hpost_retain [_ [_ [_ [_ [_
      [Hpost_joins [_ _]]]]]]]]]]]].
  split.
  - exact Hpost.
  - split.
      ...
      * split.
        ...
        -- split.
           ++ eapply frozen_caller_snapshots_nested_resume_safe_after_active_descent;
                [exact Hdescend|exact Hsnapshot_closed| |exact Hnested].
              exact (proj1 (proj2 Hexposure_wf)).
           ++ eapply frozen_completed_colors_resume_safe_after_active_descent;
                [exact Hdescend|exact Howned|exact Hexposure_wf|].
              exact Hcompleted.
Qed.

Lemma private_frozen_statement_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    T x sGamma',
    private_frozen_statement_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  ...
  have Hpost : principled_frozen_authority_history_state CT P Z cutoff
      ...
      (advance_frozen_caller_snapshots CT h
        (mk_watched_frame authority sGamma'
          (set_vars rGamma (vars rGamma ++ [Null_a]))) snapshots) h by
    (eapply principled_frozen_authority_history_after_local; eauto).
  have Hpost_parts := Hpost.
  destruct Hpost_parts as
    [_ [_ [_ [_ [Hpost_retain [_ [_ [_ [_ [_
...
Lemma private_fresh_frozen_statement_after_assignment :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x expression old value,
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x expression old value
    [Hprivate [Hcomponents [Hprospective Hafter]]] Htyping Hscope Hvalue
    Heval.
  ...
  split.
  - eapply private_frozen_statement_after_assignment; eauto.
  - split.
    + eapply frozen_callee_side_components_after_assignment; eauto.
    + split.
      * eapply frozen_callee_side_prospective_components_after_assignment;
          eauto.
      * eapply advance_snapshot_boundaries_after_cutoff. exact Hafter.
Qed.

Lemma private_fresh_frozen_statement_after_local :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    T x sGamma',
    private_fresh_frozen_statement_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    T x sGamma' [Hprivate [Hcomponents [Hprospective Hafter]]] Htyping Hnone.
  have Hmain := proj1 Hprivate.
  have Hstate := proj1 Hmain.
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hstate))))).
  have Hsound : authority_context_sound h rGamma authority :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 (proj2 Hstate)))))).
  split.
  - eapply private_frozen_statement_after_local; eauto.
  - split.
    + eapply frozen_callee_side_components_after_local; eauto.
    + split.
      * eapply frozen_callee_side_prospective_components_after_local; eauto.
      * eapply advance_snapshot_boundaries_after_cutoff. exact Hafter.
Qed.

Lemma private_frozen_statement_after_field_write :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x field y sGamma' rGamma' h',
    private_frozen_statement_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  ...
  have Hstate_parts := Hstate.
  destruct Hstate_parts as
    [_ [_ [Hruntime [Hclosed [_ [_ [_ [_ [Hexposure [Hresume _]]]]]]]]]].
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hmain))))).
  have Heffect := typed_field_write_component_effect CT authority sGamma mt
    rGamma h x field y sGamma' rGamma' h' Hwf Htyping Hscope Heval.
  ...
  assert (HrGamma : rGamma' = rGamma) by
    (inversion Heval; reflexivity).
  subst sGamma' rGamma'.
  have Hpost := principled_frozen_authority_history_after_field_write CT P Z
    cutoff authority sGamma mt rGamma h stack incoming snapshots x field y
    sGamma rGamma h' Hstate Htyping Hscope Heval.
  have Hpost_parts := Hpost.
  destruct Hpost_parts as
    [_ [_ [_ [_ [Hpost_retain [_ [_ [_ [_ [_
      [Hpost_joins [_ _]]]]]]]]]]]].
      ...
      * have Hactive_runtime : authority_colors_runtime_mutable h
        ...
        -- destruct Heffect as
              ...
              ** eapply frozen_caller_snapshots_nested_resume_safe_after_graph_reflection;
                   eauto.
              ** subst h'.
                 eapply frozen_caller_snapshots_nested_resume_safe_after_safe_field_update;
                   eauto.
        -- destruct Heffect as
                [[Hruntimes [Hmutable [Hretained Howned]]] |
                 [lx [old [written [Hheap [Hobj Hendpoints]]]]]].
              ** eapply frozen_completed_colors_resume_safe_after_graph_reflection;
                   eauto.
              ** subst h'.
...
Lemma private_frozen_statement_after_new :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x qc C args sGamma' rGamma' h',
    private_frozen_statement_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  ...
  have Hstate_parts := Hstate.
  destruct Hstate_parts as
    [_ [_ [Hruntime [Hclosed [_ [_ [_ [Hroots [Hexposure [Hresume _]]]]]]]]]].
  have Hwf : wf_r_config CT sGamma rGamma h :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 Hmain))))).
  have Hsound : authority_context_sound h rGamma authority :=
    proj1 (proj1 (proj2 (proj2 (proj2 (proj2 (proj2 Hmain)))))).
  have Hpost := principled_frozen_authority_history_after_new CT P Z cutoff
    authority sGamma mt rGamma h stack incoming snapshots x qc C args
    sGamma' rGamma' h' Hstate Htyping Heval.
  inversion Heval; subst.
  assert (Hupdate :
      set_vars rGamma (update x (Iot (dom h)) (vars rGamma)) =
      update_r_env_value rGamma x (Iot (dom h))).
  ...
  - split.
    ...
    + split.
      * apply advance_frozen_caller_snapshots_nested_covered. exact Hcovered.
      * split.
        -- eapply frozen_caller_snapshots_nested_resume_safe_after_new
             with (cutoff := cutoff) (qreceiver := qthisr)
               (qruntime := new_runtime);
             eauto.
        -- eapply frozen_completed_colors_resume_safe_after_new
             with (cutoff := cutoff) (qreceiver := qthisr)
               (qruntime := new_runtime);
             eauto.
Qed.
...
Lemma active_mutable_rdm_components_after_new :
  forall CT cutoff authority sGamma mt rGamma h x qc C args sGamma'
    vals qruntime,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SNew x qc C args) sGamma' ->
    runtime_lookup_list rGamma args = Some vals ->
    ...
    active_mutable_rdm_components_after_cutoff CT
      ...
      (mk_watched_frame authority sGamma'
        (update_r_env_value rGamma x (Iot (dom h)))).
Proof.
  intros CT cutoff authority sGamma mt rGamma h x qc C args sGamma'
    vals qruntime Hwf Htyping Hvals Hpost_heap Hcutoff Hcomponents root
    target Hroot Hroot_runtime Hreachable.
  destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C args
    sGamma' root Hwf Htyping Hroot) as
    [Hold_root | [Hroot_fresh Hrdm_creation]].
  - destruct Hold_root as
      [variable [T [Htype [Hvalue Hqualifier]]]].
    have Hroot_dom : root < dom h :=
      wf_config_value_dom CT sGamma rGamma h variable root Hwf Hvalue.
    destruct (mutable_reachable_from_old_after_append CT h
      (mkObj (mkruntime_type qruntime C) vals) root target
      (proj1 (proj2 Hwf)) Hroot_dom Hreachable) as
      [Htarget_dom Hold_reachable].
    eapply Hcomponents with (root := root).
    + exists variable, T. split; [exact Htype|].
      split; [exact Hvalue|exact Hqualifier].
    + eapply r_muttype_app_preserve_old_Some; eauto.
  ...
  - subst root qc. simpl in Hroot.
    destruct (fresh_mutable_reachable_has_creation_ancestor CT sGamma mt
      rGamma h x RDM_c C args sGamma' vals qruntime target Hwf Htyping
      Hvals Hreachable) as
      [Htarget_fresh | [old_root [Hold_root Hold_reachable]]].
    + subst target. exact Hcutoff.
    + simpl in Hold_root.
      destruct Hold_root as
        [variable [T [Htype [Hvalue Hqualifier]]]].
      have Hold_root_dom : old_root < dom h :=
        wf_config_value_dom CT sGamma rGamma h variable old_root Hwf Hvalue.
      have Htarget_dom : target < dom h :=
        mutable_reachable_target_dom CT h old_root target
          (proj1 (proj2 Hwf)) Hold_root_dom Hold_reachable.
      have Htarget_runtime_post :
          r_muttype (h ++ [mkObj (mkruntime_type qruntime C) vals]) target =
            Some Mut_r.
      { eapply mutable_reachable_preserves_runtime_mutability;
          [exact Hpost_heap|exact Hreachable|exact Hroot_runtime]. }
      have Htarget_runtime_old : r_muttype h target = Some Mut_r :=
        r_muttype_app_preserve_old_Some h
          (mkObj (mkruntime_type qruntime C) vals) target Mut_r
          Htarget_dom Htarget_runtime_post.
      have Hold_root_runtime : r_muttype h old_root = Some Mut_r.
      { eapply mutable_connected_preserves_runtime_mutability.
        - exact (proj1 (proj2 Hwf)).
        - eapply mutable_connected_sym.
          eapply mutable_reachable_connected. exact Hold_reachable.
        - exact Htarget_runtime_old. }
      eapply Hcomponents with (root := old_root).
      * exists variable, T. repeat split; assumption.
...
Lemma frozen_callee_side_components_after_new :
  forall CT authority sGamma mt rGamma h stack snapshots x qc C args
    sGamma' vals qruntime,
    wf_r_config CT sGamma rGamma h ->
    live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) stack ->
    ...
    frozen_callee_side_mutable_components_after_boundaries CT
      ...
      (advance_frozen_caller_snapshots CT
        (h ++ [mkObj (mkruntime_type qruntime C) vals])
        (mk_watched_frame authority sGamma'
          (update_r_env_value rGamma x (Iot (dom h)))) snapshots) stack.
Proof.
  intros CT authority sGamma mt rGamma h stack snapshots x qc C args
    sGamma' vals qruntime Hwf Hframes Hcutoffs Htyping Hvals Hpost_heap Hold
    snapshot boundary above below Hpartition.
  ...
  have Hlive_partition := frozen_snapshot_partition_is_live_call_partition
    (mk_watched_frame authority sGamma rGamma) snapshots stack old_snapshot
    boundary above below Hold_partition.
  have Habove_frames := live_call_partition_above_frames_wf CT h
    (mk_watched_frame authority sGamma rGamma) stack boundary above below
    Hlive_partition Hframes.
  have Hshape := live_call_partition_stack_shape
    (mk_watched_frame authority sGamma rGamma) stack boundary above below
    Hlive_partition.
  have Hboundary_cutoff : boundary.(boundary_entry_cutoff) <= dom h.
  { rewrite Hshape in Hcutoffs. apply Forall_app in Hcutoffs.
    exact (Forall_inv (proj2 Hcutoffs)). }
  have Hold_components := Hold old_snapshot boundary above below
    Hold_partition.
  eapply live_mutable_authority_components_after_new; eauto.
Qed.

Lemma frozen_callee_side_prospective_components_after_new :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x qc C args sGamma' vals qruntime,
    principled_phased_authority_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming h ->
    principled_phased_authority_live_history_state CT P Z cutoff
      (mk_watched_frame authority sGamma'
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x qc C args sGamma' vals qruntime Hstate Hpost_state Htyping Hvals Hold
    snapshot boundary above below Hpartition.
  set (post_frame := mk_watched_frame authority sGamma'
    (update_r_env_value rGamma x (Iot (dom h)))).
  destruct (advance_frozen_snapshot_live_partition_reflects CT
    (h ++ [mkObj (mkruntime_type qruntime C) vals]) post_frame snapshots stack
    snapshot boundary above below Hpartition) as
    [old_snapshot Hold_partition].
  have Hlive_partition := frozen_snapshot_partition_is_live_call_partition
    (mk_watched_frame authority sGamma rGamma) snapshots stack old_snapshot
    boundary above below Hold_partition.
  have Hpost_live_partition := live_call_partition_change_active
    (mk_watched_frame authority sGamma rGamma) post_frame stack boundary above
    below Hlive_partition.
  ...
  have Hcutoffs := proj2 (proj2 (proj2 (proj2 (proj2 (proj2 (proj2
    (proj2 (proj2 Hstate)))))))).
  have Hpost_frames := proj1 (proj2 (proj2 (proj2 (proj2 Hpost_state)))).
  ...
  have Habove_sounds := live_call_partition_above_frames_authority_sound h
    (mk_watched_frame authority sGamma rGamma) stack boundary above below
    Hlive_partition Hsounds.
  have Hpost_above_frames := live_call_partition_above_frames_wf CT
    (h ++ [mkObj (mkruntime_type qruntime C) vals]) post_frame stack boundary
    above below Hpost_live_partition Hpost_frames.
  have Hpost_above_sounds := live_call_partition_above_frames_authority_sound
    (h ++ [mkObj (mkruntime_type qruntime C) vals]) post_frame stack boundary
    above below Hpost_live_partition Hpost_sounds.
  have Hshape := live_call_partition_stack_shape
    (mk_watched_frame authority sGamma rGamma) stack boundary above below
    Hlive_partition.
  have Hboundary_cutoff : boundary.(boundary_entry_cutoff) <= dom h.
  ...
  have Hold_components := Hold old_snapshot boundary above below
    Hold_partition.
  unfold post_frame in *.
  eapply live_prospective_mutable_authority_components_after_new; eauto.
  - exact (proj1 Habove_frames).
  - exact (proj1 Habove_sounds).
  - exact (proj1 Hpost_above_frames).
  - exact (proj1 Hpost_above_sounds).
Qed.

Lemma private_fresh_frozen_statement_after_new :
  forall CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x qc C args sGamma' rGamma' h',
    private_fresh_frozen_statement_state CT P Z cutoff
      (mk_watched_frame authority sGamma rGamma) stack incoming snapshots h ->
...
Proof.
  intros CT P Z cutoff authority sGamma mt rGamma h stack incoming snapshots
    x qc C args sGamma' rGamma' h'
    [Hprivate [Hcomponents [Hprospective Hafter]]] Htyping Heval.
  ...
  have Hframes : live_frames_wf CT h
      (mk_watched_frame authority sGamma rGamma) stack :=
    proj1 (proj2 (proj2 (proj2 (proj2 Hmain)))).
  have Hcutoffs : live_boundary_cutoffs_valid h stack :=
    proj2 (proj2 (proj2 (proj2 (proj2 (proj2 (proj2
      (proj2 (proj2 Hmain)))))))).
  have Hpost := private_frozen_statement_after_new CT P Z cutoff authority
    sGamma mt rGamma h stack incoming snapshots x qc C args sGamma' rGamma'
    h' Hprivate Htyping Heval.
  have Hpost_state := proj1 (proj1 Hpost).
  have Hpost_wf := preservation_pico CT sGamma mt rGamma h
    (SNew x qc C args) rGamma' h' sGamma' Hwf Htyping Heval.
  inversion Heval; subst.
  assert (Hupdate :
      ...
      update_r_env_value rGamma x (Iot (dom h))).
  { destruct rGamma. reflexivity. }
  rewrite Hupdate in Hpost, Hpost_state, Hpost_wf |- *.
  split; [exact Hpost|]. split.
  - eapply frozen_callee_side_components_after_new; eauto.
    exact (proj1 (proj2 Hpost_wf)).
  - split.
    + eapply frozen_callee_side_prospective_components_after_new
        with (qruntime := vpa_mutability_object_creation qthisr qc); eauto.
    + eapply advance_snapshot_boundaries_after_cutoff. exact Hafter.
Qed.
...
Lemma frozen_snapshot_list_resume_exposure_protected_reflected_refl :
  forall Z snapshots,
    frozen_snapshot_list_resume_exposure_protected_reflected Z snapshots
      snapshots.
Proof.
  intros Z snapshots. induction snapshots as [|slot tail IH].
  - constructor.
  - constructor.
    + apply frozen_snapshot_slot_resume_exposure_protected_reflected_refl.
    + exact IH.
Qed.
...
Lemma frozen_snapshot_slot_resume_exposure_protected_reflected_trans :
  forall Z final middle initial,
    frozen_snapshot_slot_resume_exposure_protected_reflected Z final middle ->
    frozen_snapshot_slot_resume_exposure_protected_reflected Z middle initial ->
    frozen_snapshot_slot_resume_exposure_protected_reflected Z final initial.
Proof.
  intros Z [final|] [middle|] [initial|]; simpl; try contradiction; auto.
  eapply frozen_snapshot_resume_exposure_protected_reflected_trans; eauto.
Qed.

Lemma frozen_snapshot_list_resume_exposure_protected_reflected_trans :
  forall Z final middle initial,
    frozen_snapshot_list_resume_exposure_protected_reflected Z final middle ->
    frozen_snapshot_list_resume_exposure_protected_reflected Z middle initial ->
    frozen_snapshot_list_resume_exposure_protected_reflected Z final initial.
Proof.
  intros Z final middle initial Hfinal Hmiddle.
  generalize dependent initial.
  induction Hfinal; intros initial Hmiddle; inversion Hmiddle; subst.
  - constructor.
  - constructor.
    + eapply frozen_snapshot_slot_resume_exposure_protected_reflected_trans;
        eauto.
    + eapply IHHfinal; eauto.
Qed.

Lemma frozen_snapshot_list_resume_exposure_protected_reflected_in :
  forall Z final initial final_snapshot,
    ...
    exists initial_snapshot,
      ...
      frozen_snapshot_resume_exposure_protected_reflected Z final_snapshot
        initial_snapshot.
Proof.
  intros Z final initial final_snapshot Hreflection Hfinal.
  induction Hreflection; simpl in Hfinal; [contradiction|].
  destruct Hfinal as [Hhead | Htail].
  - subst x. destruct y as [initial_snapshot|]; simpl in H; try contradiction.
    exists initial_snapshot. split; [simpl; auto|exact H].
  - destruct (IHHreflection Htail) as
      [initial_snapshot [Hinitial Hsnapshot]].
    exists initial_snapshot. split; [simpl; auto|exact Hsnapshot].
Qed.

Lemma frozen_snapshot_list_resume_exposure_protected_reflected_tail :
  forall Z final_head final_tail initial_head initial_tail,
    frozen_snapshot_list_resume_exposure_protected_reflected Z
      (final_head :: final_tail) (initial_head :: initial_tail) ->
    frozen_snapshot_list_resume_exposure_protected_reflected Z final_tail
      initial_tail.
Proof.
  intros Z final_head final_tail initial_head initial_tail Hreflection.
  inversion Hreflection; subst. exact H4.
Qed.

(** Assignment and local declaration only descend the active frame's RDM
    roots.  Hence every advanced latent exposure reflects to the same
    location in the pre-step exposure; the protected-zone hypothesis is not
    needed in this case. *)
Lemma frozen_snapshot_list_resume_exposure_reflected_after_active_descent :
...
Proof.
  ...
  induction snapshots as [|slot tail IH]; simpl.
  - constructor.
  - constructor.
    + destruct slot as [snapshot|]; simpl; [|exact I].
      intros mode location Hmode [seed [Hseed Hpath]] Hprotected.
    ...
    + apply IH.
      * intros snapshot Hsnapshot. eapply Hruntime. simpl. right.
        exact Hsnapshot.
      ...
      * intros snapshot Hsnapshot. eapply Hentry. simpl. right.
        exact Hsnapshot.
      * intros snapshot root Hsnapshot. eapply Hroots. simpl. right.
        exact Hsnapshot.
Qed.

Lemma frozen_snapshot_list_resume_exposure_reflected_after_graph_reflection :
  forall CT old_h new_h Z active snapshots,
    (forall left right,
    ...
    frozen_snapshot_list_resume_exposure_protected_reflected Z
      (advance_frozen_caller_snapshots CT new_h active snapshots) snapshots.
Proof.
  intros CT old_h new_h Z active snapshots Hretained Hmutable Hexposure.
  destruct Hexposure as
    (Hruntime & Hclosed & Hdangerous & Hentry & Hroots).
  induction snapshots as [|slot tail IH]; simpl.
  - constructor.
  - constructor.
    + destruct slot as [snapshot|]; simpl; [|exact I].
      ...
      exists mode. split; [exact Hmode|].
      unfold advance_frozen_caller_snapshot in Hcolor. simpl in Hcolor.
...
(** A legal field update may add a latent exposure only along authority of
    the active frame.  If the target is protected, active-color separation
    eliminates that exceptional branch, leaving an old exposure at the same
    location. *)
Lemma frozen_snapshot_list_resume_exposure_reflected_after_safe_field_update :
...
Proof.
  intros CT h Z frame snapshots lx old field written Hobj Hexposure
    Hactive_runtime Hactive_safe Hendpoints.
  destruct Hexposure as
    (Hruntime & Hclosed & Hdangerous & Hentry & Hroots).
  induction snapshots as [|slot tail IH]; simpl.
  - constructor.
    ...
    + destruct slot as [snapshot|]; simpl; [|exact I].
      ...
      have Hcovered :=
        frozen_caller_connected_after_safe_field_update_covered_by_old_or_active
          ...
          lx old field written seed (mode, location) Hobj
          (Hruntime snapshot (ltac:(simpl; auto)))
          (Hclosed snapshot (ltac:(simpl; auto))) Hactive_runtime Hendpoints
          Hsource Hpath Hmode.
      destruct Hcovered as
        [[old_mode [Hold_mode Hold_color]] |
         [active_mode [Hactive_mode Hactive_color]]].
      * exists old_mode. split; assumption.
      * exfalso. eapply Hactive_safe; eauto.
    + apply IH.
      * intros snapshot Hsnapshot. eapply Hruntime. simpl. right.
        exact Hsnapshot.
      * intros snapshot Hsnapshot. eapply Hclosed. simpl. right.
...
Lemma frozen_snapshot_list_resume_exposure_reflected_after_new :
  forall CT Z cutoff sGamma mt rGamma h x qc C args sGamma' vals
    qreceiver qruntime authority snapshots,
    wf_r_config CT sGamma rGamma h ->
    wf_r_config CT sGamma'
...
Proof.
  intros CT Z cutoff sGamma mt rGamma h x qc C args sGamma' vals
    qreceiver qruntime authority snapshots Hwf Hpost_wf Hsound Hpost_sound
    Htyping Hvals Hadapt Hcutoff Hzone Hexposure Hactive_safe.
  destruct Hexposure as
    (Hruntime & Hclosed & Hdangerous & Hentry & Hroots).
  induction snapshots as [|slot tail IH]; simpl.
  - constructor.
  - constructor.
    + destruct slot as [snapshot|]; simpl; [|exact I].
      intros exposure_mode target Hexposure_mode Htarget Hprotected.
      have Htarget_old : target < dom h.
      { have Hbefore := Hzone target Hprotected. lia. }
      have Hpost_target : In authority_flow_state
          (executing_authority_color_set CT
      ...
      destruct (executing_authority_colors_after_new_covered CT sGamma mt
        rGamma h x qc C args sGamma' vals qreceiver qruntime authority
        snapshot.(frozen_snapshot_current_resume_exposure) Hwf Hpost_wf
        Hsound Hpost_sound (Hruntime snapshot (ltac:(simpl; auto))) Htyping
        Hvals Hadapt exposure_mode target Hexposure_mode Hpost_target
        Htarget_old) as [old_target_mode [Hold_target_mode Hold_target]].
      destruct
        (executing_with_frozen_incoming_dangerous_covered_by_old_or_active
          CT h (mk_watched_frame authority sGamma rGamma)
          snapshot.(frozen_snapshot_current_resume_exposure)
          old_target_mode target (Hclosed snapshot (ltac:(simpl; auto)))
          Hold_target_mode Hold_target) as
        [[snapshot_target_mode
            [Hsnapshot_target_mode Hsnapshot_target]] |
         [active_target_mode [Hactive_target_mode Hactive_target]]].
      * exists snapshot_target_mode. split; assumption.
      * exfalso. eapply Hactive_safe; eauto.
    + apply IH.
      * intros snapshot Hsnapshot. eapply Hruntime. simpl. right.
        exact Hsnapshot.
