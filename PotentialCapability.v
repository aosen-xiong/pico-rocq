...
Lemma potential_adjacent_symmetric :
  forall CT h active stack left right,
    potential_adjacent CT h active stack left right ->
    potential_adjacent CT h active stack right left.
Proof.
  intros CT h active stack left right [Hheap | [Hframe | Hreturn]].
...
Lemma potential_connected_sym :
  forall CT h active stack left right,
    potential_connected CT h active stack left right ->
    potential_connected CT h active stack right left.
Proof.
  intros CT h active stack left right Hconnected.
  induction Hconnected.
  - apply rt_step. eapply potential_adjacent_symmetric; eauto.
  - apply rt_refl.
...
Proof.
  ...
  - inversion Hlive; subst.
    + destruct (Hdescend left Hleft) as
      ...
      eapply potential_connected_trans.
      * eapply potential_connected_sym.
...
Proof.
  ...
  - destruct (mutable_adjacent_after_field_update CT h lx old field value
      left right Hobj Hheap) as [Hold | Hnew].
...
(** A field update adds at most one undirected potential edge.  This is the
...
Proof.
  ...
  - destruct IHHconnected1 as
    ...
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      left. eapply potential_connected_trans; [exact Hxlx|].
      eapply potential_connected_trans.
      * eapply potential_connected_sym.
        eapply potential_connected_trans; [exact Hwritten1y|exact Hylx].
      * exact Hwritten2z.
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      left. eapply potential_connected_trans; eauto.
    ...
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      left. eapply potential_connected_trans; eauto.
    + rewrite Hvalue1 in Hvalue2. injection Hvalue2 as <-.
      left. eapply potential_connected_trans; [exact Hxwritten1|].
      eapply potential_connected_trans.
      * eapply potential_connected_sym.
        eapply potential_connected_trans; [exact Hlxy|exact Hywritten2].
      * exact Hlxyz.
Qed.
...
Lemma potential_connected_after_nonlocation_field_update_is_old :
  forall CT h active stack lx old field value left right,
    runtime_getObj h lx = Some old ->
    (forall location, value <> Iot location) ->
    potential_connected CT (update_field h lx field value)
      active stack left right ->
    potential_connected CT h active stack left right.
Proof.
  intros CT h active stack lx old field value left right Hobj Hnonlocation
    Hconnected.
  eapply potential_connected_map_edges; [|exact Hconnected].
  intros edge_left edge_right [Hheap | [Hframe | Hreturn]].
  - destruct (mutable_adjacent_after_field_update CT h lx old field value
      edge_left edge_right Hobj Hheap) as
      [Hold | [written [Hvalue _]]].
    + apply rt_step. left. exact Hold.
...
Proof.
  intros CT h active stack lx old field value C fieldT location Hobj
    Hbase Hfield Hnot_rdm [root [Hroot Hreachable]].
...
Lemma live_capability_after_nonlocation_field_update_is_old :
  forall CT h active stack lx old field value location,
    runtime_getObj h lx = Some old ->
    (forall target, value <> Iot target) ->
    live_capability_reachable CT (update_field h lx field value)
      active stack location ->
    live_capability_reachable CT h active stack location.
Proof.
  intros CT h active stack lx old field value location Hobj Hnonlocation
...
Proof.
  ...
  - injection Hvalue as <-. eapply potential_connected_trans.
    + exact Hleft_written.
    + eapply potential_connected_trans.
      * eapply potential_connected_sym. exact Hredundant.
      * exact Hlx_right.
Qed.
...
Proof.
  intros CT h active stack lx old field written M' M Z Hobj Hredundant
...
Proof.
  ...
  - have Hroot_live : In Loc (live_capability_set CT h active stack) root.
    ...
    have Hlx_runtime := mutable_reachable_preserves_runtime_mutability CT h
      root lx Mut_r Hheap_wf Hroot_lx Hroot_runtime.
...
Proof.
  ...
  have Hlive_post := live_history_after_field_write CT P Z cutoff authority
    sGamma mt rGamma h stack x field y sGamma' rGamma' h' Hlive Htyping
    Hscope Heval.
  split; [exact Hlive_post|].
  ...
  destruct val_y as [|written|n].
  - eapply potential_colors_after_graph_reflection
      with (M := live_capability_set CT h
        (mk_watched_frame authority sGamma rGamma) stack).
    + intros location Hlocation.
      eapply live_capability_after_nonlocation_field_update_is_old; eauto;
        discriminate.
    + intros left right Hconnected.
      eapply potential_connected_after_nonlocation_field_update_is_old; eauto;
        discriminate.
    + exact Hpotential.
  - destruct (typed_field_write_runtime_field_agreement CT sGamma mt rGamma
      ...
      [Tx [C [fieldT [Hgetx [Href [Hfield_definition Hruntime_base]]]]]].
    ...
    + eapply potential_colors_after_graph_reflection
      ...
      * intros location Hlocation.
        eapply live_capability_after_non_rdm_field_update_is_old
          with (C := C) (fieldT := fieldT); eauto; discriminate.
      * intros left right Hconnected.
    ...
    + destruct (typed_runtime_rdm_field_write_subtyping CT sGamma mt rGamma
        h x field y loc_x o C fieldT sGamma Hwf Htyping Hscope
...
Proof.
  ...
  - right. right. split; [reflexivity|assumption].
  - apply rt_refl.
Qed.
...
Proof.
  ...
  - have Hheap_wf : wf_heap CT h := proj1 (proj2 Hwf).
    have Hheap_connected : mutable_connected CT
        (h ++ [mkObj (mkruntime_type qruntime C) vals]) left right.
    { apply rt_step. exact Hheap. }
    destruct (mutable_connected_after_append_components CT h
      (mkObj (mkruntime_type qruntime C) vals) left right Hheap_wf
      Hheap_connected) as [Hold | [Hleft Hright]].
    + left. eapply mutable_connected_is_potential_connected; eauto.
    + right. split;
        eapply fresh_component_attachment_is_potential_attachment; eauto.
  - destruct Hframe as [frame [Hlive [Hleft Hright]]].
    ...
    + destruct (new_active_rdm_root_origin CT sGamma mt rGamma h x qc C
      ...
      * subst qc. right. split.
        -- apply potential_new_attachment_typed_root. exact Hleft_old.
        -- subst right. apply potential_new_attachment_fresh.
      * subst qc. right. split.
        -- subst left. apply potential_new_attachment_fresh.
...
Proof.
  intros CT sGamma mt rGamma h x qc C args sGamma' vals qruntime
    ...
    Hconnected.
  induction Hconnected.
  - eapply potential_adjacent_after_new; eauto.
  - left. apply rt_refl.
  - destruct IHHconnected1 as [Hxy | [Hattach_x Hattach_y]];
      destruct IHHconnected2 as [Hyz | [Hattach_y' Hattach_z]].
    + left. eapply potential_connected_trans; eauto.
    + right. split.
      * eapply potential_new_attachment_transport.
        -- exact Hattach_y'.
        -- eapply potential_connected_sym. exact Hxy.
      * exact Hattach_z.
    + right. split.
      * exact Hattach_x.
      * eapply potential_new_attachment_transport; [exact Hattach_y|exact Hyz].
...
Proof.
  intros CT h active stack qc root Hframes Hroot
    [anchor [[Hfresh | [Htyped | Hcaller]] Hconnected]].
  - subst anchor.
  ...
  - exists anchor. split; [right; exact Hcaller|exact Hconnected].
Qed.
...
Proof.
  ...
  destruct (potential_connected_after_new CT sGamma mt rGamma h x qc C
    ...
    [Hold_connected | [Hcapability_attachment Hprotected_attachment]].
  ...
  - destruct (potential_new_attachment_to_old_has_typed_anchor CT h
    ...
    destruct qc.
    + destruct Hzone_anchor as [Hzone_anchor | [Himpossible Hcaller]].
      2: discriminate.
    ...
    + destruct Hzone_anchor as [Hzone_anchor | [Himpossible Hcaller]].
      2: discriminate.
      simpl in Hzone_anchor.
      have Hcapability_attachment_copy := Hcapability_attachment.
      assert (Hcapability_old : capability < dom h).
      { destruct Hcapability_attachment as
          [anchor [[Hanchor_fresh | [Hanchor_typed |
            [Himpossible Hcaller]]] Hanchor_connected]].
        - subst anchor.
          have Hcapability_fresh := potential_connected_from_fresh_is_fresh
            CT h (mk_watched_frame authority sGamma rGamma) stack capability
            Hframes Hanchor_connected.
          subst capability.
          have Hfresh_runtime_live := live_capability_members_runtime_mutable
            CT (h ++ [mkObj (mkruntime_type new_runtime C) vals])
            (mk_watched_frame authority sGamma'
              (update_r_env_value rGamma x (Iot (dom h)))) stack
            Hpost_frames Hpost_sound (dom h) Hcapability.
          destruct qthisr; unfold new_runtime in Hfresh_runtime_live;
            simpl in Hfresh_runtime_live;
            unfold r_muttype, r_type in Hfresh_runtime_live;
            rewrite runtime_getObj_last in Hfresh_runtime_live;
            discriminate.
        - destruct Hanchor_typed as
            [variable [T [Htype [Hvalue Hqualifier]]]].
          have Hanchor_dom := wf_config_value_dom CT sGamma rGamma h
            variable anchor Hwf Hvalue.
          eapply potential_connected_left_dom_from_right.
          * exact Hframes.
          * eapply potential_connected_sym. exact Hanchor_connected.
          * exact Hanchor_dom.
        - discriminate. }
      destruct (potential_new_attachment_to_old_has_typed_anchor CT h
        (mk_watched_frame authority sGamma rGamma) stack Imm_c capability
        Hframes Hcapability_old Hcapability_attachment_copy) as
        [capability_anchor [Hcapability_anchor Hanchor_connected]].
      destruct Hcapability_anchor as
        [Hcapability_anchor | [Himpossible Hcaller]].
      2: discriminate.
      simpl in Hcapability_anchor.
      have Hcapability_pre : In Loc
          (live_capability_set CT h
            (mk_watched_frame authority sGamma rGamma) stack) capability.
      { eapply new_live_reachability_to_old_location_has_old_origin
          with (mt := mt) (x := x) (qc := Imm_c) (C := C) (args := args)
            (sGamma' := sGamma') (vals := vals) (qruntime := new_runtime);
          eauto. }
      have Hcapability_runtime := live_capability_members_runtime_mutable CT h
        (mk_watched_frame authority sGamma rGamma) stack Hframes Hsound
        capability Hcapability_pre.
      ...
      have Hanchor_runtime := potential_connected_preserves_runtime_mutability
        CT h (mk_watched_frame authority sGamma rGamma) stack capability
        capability_anchor Mut_r Hframes Hheap_wf
        (potential_connected_sym CT h
          (mk_watched_frame authority sGamma rGamma) stack
          capability_anchor capability Hanchor_connected)
        Hcapability_runtime.
      rewrite Hanchor_immutable in Hanchor_runtime. discriminate.
    + simpl in Hzone_anchor.
      assert (Hzone_creation :
        typed_root RDM sGamma rGamma zone_anchor \/
        immediate_rdm_caller_root h
          (mk_watched_frame authority sGamma rGamma) stack zone_anchor).
      { destruct Hzone_anchor as [Hzone_active | [Hzone_qc Hzone_caller]].
        - left. exact Hzone_active.
        - right. exact Hzone_caller. }
      destruct Hcapability_attachment as
        [capability_anchor
      ...
      * subst capability_anchor.
        have Hcapability_fresh := potential_connected_from_fresh_is_fresh CT h
...
Lemma potential_adjacent_after_call_push :
  forall CT h boundary stack callee_authority left right,
    ...
    (boundary_view_attachment CT h boundary stack left /\
...
Proof.
  ...
  - destruct Hframe as [frame [Hlive [Hleft Hright]]].
    ...
    + have Hleft_origin := boundary_entry_rdm_root_by_view boundary left Hleft.
      ...
      * right. split; apply boundary_view_attachment_root;
...
Proof.
  ...
  - destruct IHHconnected1 as [Hxy | [Hattach_x Hattach_y]];
    ...
    + left. eapply potential_connected_trans; eauto.
    + right. split.
      * eapply boundary_view_attachment_transport.
        -- exact Hattach_y'.
        -- eapply potential_connected_sym. exact Hxy.
      * exact Hattach_z.
    + right. split.
...
Lemma potential_history_enter_call :
  forall CT P Z cutoff caller_authority sGamma mt rGamma h stack
    x y method args sGamma' vals ly cy runtime_mdef Ty,
    potential_live_history_state CT P Z cutoff
...
Proof.
  ...
  destruct (live_history_enter_call CT P Z cutoff caller_authority sGamma mt
    rGamma h stack x y method args sGamma' vals ly cy runtime_mdef Ty Hlive
  ...
  { unfold boundary, caller, callee_senv, callee_renv in Hcapability |- *.
    apply (proj1 (call_push_live_reachability_equivalent CT caller_authority
      sGamma mt rGamma h x y method args sGamma' vals ly cy runtime_mdef Ty
  ...
  - destruct (sqtype Ty) eqn:Hview.
    ...
    + destruct Hcapability_attachment as
      ...
      have Hanchor_runtime := potential_connected_preserves_runtime_mutability
        ...
        (potential_connected_sym CT h caller stack capability_anchor
          capability Hanchor_connected) Hcapability_runtime.
      rewrite Hanchor_immutable in Hanchor_runtime. discriminate.
    + destruct Hcapability_attachment as
      ...
      eapply potential_connected_trans.
      * eapply potential_connected_sym. exact Hcapability_connected.
      * eapply potential_connected_trans.
        -- eapply live_frame_rdm_roots_potentially_connected
...
Lemma caller_nonlocation_rdm_roots_descend :
  forall CT caller_senv caller_renv h destination destination_type return_value,
...
Proof.
  intros CT caller_senv caller_renv h destination destination_type return_value
    Hnonlocation Hwf
...
Lemma potential_history_leave_call_nonlocation :
  forall CT P Z cutoff caller_authority caller_senv caller_renv caller_h stack
...
Proof.
  ...
  { have Hcaller_boundary_wf := Forall_inv (proj2 Hpre_frames).
    change (wf_r_config CT caller_senv caller_renv callee_h)
      in Hcaller_boundary_wf.
    exact Hcaller_boundary_wf. }
  have Hdescend := caller_nonlocation_rdm_roots_descend CT caller_senv caller_renv
    callee_h destination destination_type return_value Hnonlocation
    Hcaller_current_wf Hdestination_type.
  set (Mpre := live_capability_set CT callee_h callee_frame
    (caller_boundary :: stack)).
  assert (Hpost_separated_pre :
    potential_colors_separated CT callee_h Mpre Z caller_post stack).
  ...
  have Hcaller_colors : watched_frame_colors CT callee_h Mpre Z caller_post.
  { eapply potential_colors_imply_active_colors; eauto. }
  have Hlive_post := live_history_leave_call_nonlocation_given_caller_colors CT P Z
    ...
    callee_renv callee_h return_value Hnonlocation Hcaller_zone Hcaller_env Hcaller_wf
...
Lemma safe_typed_call_target_method_safe :
  forall CT sGamma mt rGamma h x y method args sGamma' ly cy runtime_mdef,
    wf_r_config CT sGamma rGamma h ->
    stmt_typing CT sGamma mt (SCall x y method args) sGamma' ->
    safe_readonly_method_type mt ->
    runtime_getVal rGamma y = Some (Iot ly) ->
...
Proof.
  intros CT sGamma mt rGamma h x y method args sGamma' ly cy runtime_mdef
...
Proof.
  ...
  - destruct Hfind as [Hfind_method Hbody_definition].
    ...
    destruct retval as [|return_location|n].
    + eapply potential_history_leave_call_nonlocation with
        ...
        (callee_renv := rΓ'') (callee_h := h') (return_value := Null_a);
        eauto; discriminate.
