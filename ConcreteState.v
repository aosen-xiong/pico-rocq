...
Lemma runtime_and_static_method_signatures_agree :
  forall CT sΓ rΓ h y loc Ty staticClass runtimeClass m
    mdefRuntime mdefStatic,
    wf_r_config CT sΓ rΓ h ->
    static_getType sΓ y = Some Ty ->
    sbase Ty = TRef staticClass ->
    runtime_getVal rΓ y = Some (Iot loc) ->
    r_basetype h loc = Some runtimeClass ->
    FindMethodWithName CT runtimeClass m mdefRuntime ->
    FindMethodWithName CT staticClass m mdefStatic ->
    msignature mdefRuntime = msignature mdefStatic.
Proof.
  intros CT sΓ rΓ h y loc Ty staticClass runtimeClass m
    mdefRuntime mdefStatic Hwf Hget_y Href Hval_y Hbase Hfind_runtime
    Hfind_static.
  have Hwf_copy := Hwf.
  ...
  simpl in Hbase_sub.
  rewrite Href in Hbase_sub.
  destruct (Subtyping.base_subtype_from_ref CT (rctype (rt_type obj))
    (TRef staticClass)
    Hbase_sub) as [D [HD Hclass_sub]].
  injection HD as <-.
